...
std::function<bool(const std::string&)>WsSendJsonSCLogger = [](const std::string& s) -> bool {
...
static void readWebUIJson(struct mg_connection* conn, char* data, size_t len) {
  if (!len) return sendPlaybackState(conn);
  ...
  if (j["type"] == "cmd") {
    if (j["cmd"] == "play") {
      if (feedControl) feedControl->feedCommand(AudioControl::CommandType::PLAY, 0);
    }
    else if (j["cmd"] == "pause") {
      if (feedControl) feedControl->feedCommand(AudioControl::CommandType::PAUSE, 0);
    }
    else if (j["cmd"] == "set_volume") {
      if (feedControl) feedControl->feedCommand(AudioControl::CommandType::VOLUME_LINEAR,
        j["value"].get<uint8_t>(),
    ...
    if (j["cmd"] == "play_station") {
      ...
      for (auto& s : r.fields) {
        if (s.name == j["station"]["name"].get<std::string>()) {
          s = { j["station"]["name"].get<std::string>(), j["station"]["url"].get<std::string>() };
          radioStore.save(r);
          return;
        }
      }
      if (r.userkey.empty()) r.userkey = "stations";
      r.fields.push_back({ j["station"]["name"].get<std::string>(), j["station"]["url"].get<std::string>() });
      radioStore.save(r);
    }
    ...
    if (j["page"] == "page-radio") {
      ...
      for (auto& s : stations.fields) {
        j["stations"].push_back({ {"name",s.name},{"url",std::string(s.value.begin(),s.value.end())} });
      }
      ...
      if (esp_wifi_sta_get_ap_info(&info) == ESP_OK) {
        ...
        taskCount = uxTaskGetSystemState(tasks.data(), taskCount, &totalRunTime);
...
void app_main(void) {
  ...
  if (!timesync::wait_until_valid(8000)) {
    BELL_LOG(error, "QOBUZ", "System time not valid; cannot call API needing request_ts");
  }
  ...
  spotify_app = std::make_unique<SpotifyStream>(audioControl, std::make_unique<SecureStore>("spotify"), [](bool connected) {
    if (connected) {
      if (current_streaming_service) {
        onEndOfStream();
      }
