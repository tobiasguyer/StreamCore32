...
namespace {
  ...
  inline void strip_nvs_cstr(std::string& s, size_t nvs_reported_len_including_nul) {
    if (nvs_reported_len_including_nul > 0 && nvs_reported_len_including_nul <= s.size())
  ...
  inline std::string b64encode(const std::vector<uint8_t>& in) {
    ...
    std::string out;
    out.resize(olen);
    if (mbedtls_base64_encode(reinterpret_cast<unsigned char*>(&out[0]), out.size(), &olen, in.data(), in.size()) != 0)
      return {};
    out.resize(olen);
    return out;
  }
  inline std::vector<uint8_t> b64decode(const std::string& in) {
    size_t olen = 0;
    (void)mbedtls_base64_decode(nullptr, 0, &olen, reinterpret_cast<const unsigned char*>(in.data()), in.size());
    std::vector<uint8_t> out;
    out.resize(olen);
    if (mbedtls_base64_decode(out.data(), out.size(), &olen, reinterpret_cast<const unsigned char*>(in.data()), in.size()) != 0)
      return {};
    out.resize(olen);
    return out;
  }
  ...
  inline std::string key_suffix_for(const std::string& userkey) {
    ...
    mbedtls_sha256_update_ret(&ctx, reinterpret_cast<const uint8_t*>(userkey.data()), userkey.size());
  ...
  inline std::string make_name_key(const std::string& suffix) { return std::string(kNamePref) + suffix; }
  inline std::string make_blob_key(const std::string& suffix) { return std::string(kBlobPref) + suffix; }
  ...
  inline esp_err_t gcm_encrypt(const std::vector<uint8_t>& key32,
    ...
    if (key32.size() != 32) return ESP_ERR_INVALID_ARG;

    // blob = MAGIC(4) | IV(12) | CIPHERTEXT | TAG(16)
    uint8_t iv[kGcmIvLen];
    for (size_t i = 0;i < kGcmIvLen;++i) iv[i] = (uint8_t)esp_random();
  ...
  inline esp_err_t gcm_decrypt(const std::vector<uint8_t>& key32,
    ...
    if (key32.size() != 32) return ESP_ERR_INVALID_ARG;
    if (!blob_is_encrypted(blob, len)) return ESP_ERR_INVALID_ARG;
    if (len < 4 + kGcmIvLen + kGcmTagLen) return ESP_ERR_INVALID_SIZE;
    ...
    int rc = mbedtls_gcm_auth_decrypt(&gcm, data_len,
      iv, kGcmIvLen,
      nullptr, 0,
      tag, kGcmTagLen,
      data, pt.data());
  ...
  inline esp_err_t find_suffix_for_userkey(nvs_handle_t h, const char* ns, const std::string& userkey, std::string& out_suf) {
    ...
    while (err == ESP_OK && it != nullptr) {
      ...
      if (std::strncmp(info.key, kNamePref, 2) != 0) continue;

      size_t len = 0;
      if (nvs_get_str(h, info.key, nullptr, &len) != ESP_OK) continue;
      std::string u(len, '\0');
      if (nvs_get_str(h, info.key, &u[0], &len) != ESP_OK) continue;
...
namespace {
  ...
  inline esp_err_t run_in_nvs_task(esp_err_t(*fn)(void*), void* arg) {
    ...
    NvsTaskCtx* ctx = static_cast<NvsTaskCtx*>(
      heap_caps_malloc(sizeof(NvsTaskCtx), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(
      nvs_worker_task, "nvsw",
      NVS_WORKER_TASK_STACK,
      ctx,
      NVS_WORKER_TASK_PRIO,
      &th,
      NVS_WORKER_TASK_CORE);
    if (ok != pdPASS) {
      heap_caps_free(ctx);
...
esp_err_t init_nvs(const uint8_t* key, size_t len) {
  ...
  if (err == ESP_OK) {
    ...
    else g_master_key.assign(key, key + len);
    g_has_key = true;
    // Notify any waiters
...
struct SaveArgs { const char* ns; Record r; bool overwrite; };
static esp_err_t w_store_save(void* pv) {
  ...
  if (!js) { cJSON_Delete(root); nvs_close(h); return ESP_ERR_NO_MEM; }

  e = nvs_set_str(h, kname.c_str(), a->r.userkey.c_str());
  if (e == ESP_OK) e = nvs_set_blob(h, kblob.c_str(), js, std::strlen(js));
  cJSON_free(js);
  cJSON_Delete(root);
...
struct LoadArgs { const char* ns; std::string userkey; Record* out; };
static esp_err_t w_store_load(void* pv) {
  auto* a = static_cast<LoadArgs*>(pv);
  if (!a->out) return ESP_ERR_INVALID_ARG;
  ...
  if (e != ESP_OK) {
    std::string suf2;
    if (find_suffix_for_userkey(h, a->ns, a->userkey, suf2) != ESP_OK) { nvs_close(h); return ESP_ERR_NOT_FOUND; }
    suf = std::move(suf2);
  ...
  size_t bl = 0;
  if (nvs_get_blob(h, kblob.c_str(), nullptr, &bl) != ESP_OK) { nvs_close(h); return ESP_ERR_NOT_FOUND; }
  std::string js; js.resize(bl);
  if (nvs_get_blob(h, kblob.c_str(), &js[0], &bl) != ESP_OK) { nvs_close(h); return ESP_FAIL; }

  cJSON* root = cJSON_ParseWithLength(js.data(), js.size());
  if (!root) { nvs_close(h); return ESP_ERR_INVALID_RESPONSE; }
...
struct ListArgs { const char* ns; std::vector<std::string>* out; };
...
struct SearchArgs {
  const char* ns; std::string q; bool ci; bool pref; std::vector<std::string>* out;
};
static esp_err_t w_store_search(void* pv) {
  ...
  esp_err_t err = nvs_entry_find(kPartName, a->ns, NVS_TYPE_STR, &it);
  while (err == ESP_OK && it != nullptr) {
    nvs_entry_info_t info; nvs_entry_info(it, &info); err = nvs_entry_next(&it);
    if (std::strncmp(info.key, kNamePref, 2) != 0) continue;
    ...
    bool match = a->pref ? (cmp.rfind(q, 0) == 0) : (q.empty() || cmp.find(q) != std::string::npos);
    if (match) a->out->push_back(std::move(u));
  }
  nvs_close(h);
  return ESP_OK;
}
esp_err_t Store::search_userkeys(const std::string& query, std::vector<std::string>& out_userkeys, bool case_insensitive, bool prefix_only) {
  SearchArgs a{ ns_, query, case_insensitive, prefix_only, &out_userkeys };
  return run_in_nvs_task(w_store_search, &a);
}

struct ExistsArgs { const char* ns; std::string userkey; esp_err_t* out; };
...
struct EraseArgs { const char* ns; std::string userkey; };
static esp_err_t w_store_erase(void* pv) {
  ...
  std::string suf;
  if (find_suffix_for_userkey(h, a->ns, a->userkey, suf) != ESP_OK) { nvs_close(h); return ESP_ERR_NOT_FOUND; }
  ...
  if (nvs_get_str(h, kCurrKey, nullptr, &l) == ESP_OK) {
    ...
    if (nvs_get_str(h, kCurrKey, &curr[0], &l) == ESP_OK) {
      ...
      if (curr == a->userkey) (void)nvs_erase_key(h, kCurrKey);
    }
  }
  e = (e1 == ESP_OK || e2 == ESP_OK) ? nvs_commit(h) : ((e1 != ESP_OK) ? e1 : e2);
  nvs_close(h);
  return e;
}
...
struct SetCurArgs { const char* ns; std::string userkey; };
...
struct GetCurArgs { const char* ns; Record* out; };
...
struct UpdFieldArgs { const char* ns; std::string userkey, fname; std::vector<uint8_t> val; bool create; };
static esp_err_t w_store_update_field(void* pv) {
  auto* a = static_cast<UpdFieldArgs*>(pv);
  if (a->fname.empty()) return ESP_ERR_INVALID_ARG;
  ...
  auto it = std::find_if(r.fields.begin(), r.fields.end(), [&](const Field& f) { return f.name == a->fname; });
  if (it == r.fields.end()) r.fields.push_back(Field{ a->fname, a->val });
  else it->value = a->val;

  SaveArgs sa{ a->ns, r, true };
  return w_store_save(&sa);
}
esp_err_t Store::update_field(const std::string& userkey, const std::string& field_name, const std::vector<uint8_t>& value, bool create_record_if_missing) {
  UpdFieldArgs a{ ns_, userkey, field_name, value, create_record_if_missing };
  return run_in_nvs_task(w_store_update_field, &a);
}

struct GetFieldArgs { const char* ns; std::string userkey, fname; std::vector<uint8_t>* out; };
static esp_err_t w_store_get_field(void* pv) {
  ...
  esp_err_t e = w_store_load(&la);
  if (e != ESP_OK) return e;
  for (auto& f : r.fields) if (f.name == a->fname) { *a->out = f.value; return ESP_OK; }
  return ESP_ERR_NOT_FOUND;
}
esp_err_t Store::get_field(const std::string& userkey, const std::string& field_name, std::vector<uint8_t>* out) {
  GetFieldArgs a{ ns_, userkey, field_name, out };
  return run_in_nvs_task(w_store_get_field, &a);
}

esp_err_t Store::set_field_str(const std::string& userkey, const std::string& field_name, const std::string& value, bool create_record_if_missing) {
  return update_field(userkey, field_name, as_bytes(value), create_record_if_missing);
}

esp_err_t Store::get_field_str(const std::string& userkey, const std::string& field_name, std::string* out) {
...
struct ExportArgs { const char* ns; const char* path; };
static esp_err_t w_store_export(void* pv) {
  auto* a = static_cast<ExportArgs*>(pv);
  if (!a->path) return ESP_ERR_INVALID_ARG;
  ...
  for (auto& u : users) {
    ...
    if (w_store_load(&l2) != ESP_OK) continue;
  ...
  if (!js) { cJSON_Delete(root); return ESP_ERR_NO_MEM; }

  FILE* f = std::fopen(a->path, "w");
  if (!f) { cJSON_free(js); cJSON_Delete(root); return ESP_FAIL; }
...
struct ImportArgs { const char* ns; const char* path; bool overwrite; };
static esp_err_t w_store_import(void* pv) {
  ...
  auto handle_obj = [&](cJSON* o) {
    ...
    if (cJSON_IsString(uk) && (cJSON_IsObject(fs) || fs == nullptr)) {
      Record r; r.userkey = uk->valuestring ? uk->valuestring : "";
  ...
  if (cJSON_IsArray(root)) {
    ...
    for (int i = 0;i < sz;++i) { cJSON* o = cJSON_GetArrayItem(root, i); if (cJSON_IsObject(o)) handle_obj(o); }
  }
...
struct FirstArgs { const char* ns; Record* out; };
static esp_err_t w_store_get_first(void* pv) {
  ...
  while (err == ESP_OK && it != nullptr) {
    ...
    if (std::strncmp(info.key, kNamePref, 2) == 0) {
      ...
      if (best_suf.empty() || make_name_key(suf) < make_name_key(best_suf)) best_suf = std::move(suf);
    }
  }

  if (best_suf.empty()) { nvs_close(h); return ESP_ERR_NOT_FOUND; }

  const std::string kname = make_name_key(best_suf);
  const std::string kblob = make_blob_key(best_suf);

  size_t len = 0;
  if (nvs_get_str(h, kname.c_str(), nullptr, &len) != ESP_OK) { nvs_close(h); return ESP_ERR_NOT_FOUND; }
  std::string u(len, '\0');
  if (nvs_get_str(h, kname.c_str(), &u[0], &len) != ESP_OK) { nvs_close(h); return ESP_FAIL; }
  strip_nvs_cstr(u, len);
...
struct StartupArgs { const char* ns; bool setcur; Record* out; };
static esp_err_t w_store_get_startup(void* pv) {
  ...
  if (w_store_getcur(&ga) == ESP_OK) return ESP_OK;
...
struct SSaveArgs { const char* ns; Record r; bool overwrite; };
static esp_err_t w_secure_save(void* pv) {
  auto* a = static_cast<SSaveArgs*>(pv);
  if (!g_has_key || g_master_key.size() != 32) return ESP_ERR_INVALID_STATE;
  ...
  e = nvs_set_str(h, kname.c_str(), a->r.userkey.c_str());
  if (e == ESP_OK) e = nvs_set_blob(h, kblob.c_str(), payload.data(), payload.size());
  if (e == ESP_OK) e = nvs_commit(h);
  nvs_close(h);
  return e;
}
...
struct SLoadArgs { const char* ns; std::string userkey; Record* out; };
static esp_err_t w_secure_load(void* pv) {
  ...
  nvs_handle_t h{};
  esp_err_t e = nvs_open(a->ns, NVS_READONLY, &h);
  if (e != ESP_OK) printf("SecureStore::w_secure_load() => %s\n", esp_err_to_name(e));
  if (e != ESP_OK) return e;
  ...
  if (nvs_get_blob(h, kblob.c_str(), blob.data(), &bl) != ESP_OK) { nvs_close(h); return ESP_FAIL; }
  nvs_close(h);

  if (e != ESP_OK) printf("SecureStore::w_secure_load() => %s\n", esp_err_to_name(e));
  if (!blob_is_encrypted(blob.data(), blob.size())) return ESP_ERR_INVALID_RESPONSE;
...
struct SListArgs { const char* ns; std::vector<std::string>* out; };
...
struct SSrchArgs { const char* ns; std::string q; bool ci; bool pref; std::vector<std::string>* out; };
static esp_err_t w_secure_search(void* pv) { return w_store_search(pv); }
esp_err_t SecureStore::search_userkeys(const std::string& query, std::vector<std::string>& out_userkeys, bool case_insensitive, bool prefix_only) {
  SSrchArgs a{ ns_, query, case_insensitive, prefix_only, &out_userkeys };
  return run_in_nvs_task(w_secure_search, &a);
}

struct SExistsArgs { const char* ns; std::string userkey; esp_err_t* out; };
static esp_err_t w_secure_exists(void* pv) { return w_store_exists(pv); }
...
struct SEraseArgs { const char* ns; std::string userkey; };
static esp_err_t w_secure_erase(void* pv) { return w_store_erase(pv); }
esp_err_t SecureStore::erase(const std::string& userkey) {
  SEraseArgs a{ ns_, userkey };
  return run_in_nvs_task(w_secure_erase, &a);
}

struct SSetCurArgs { const char* ns; std::string userkey; };
static esp_err_t w_secure_setcur(void* pv) { return w_store_setcur(pv); }
...
struct SGetCurArgs { const char* ns; Record* out; };
static esp_err_t w_secure_getcur(void* pv) {
  ...
  nvs_handle_t h{};
  esp_err_t e = nvs_open(a->ns, NVS_READONLY, &h);
  if (e != ESP_OK) printf("SecureStore::w_secure_get_current() => %s\n", esp_err_to_name(e));
  if (e != ESP_OK) return e;

  size_t l = 0;
  e = nvs_get_str(h, kCurrKey, nullptr, &l);
  if (e != ESP_OK) { nvs_close(h); printf("SecureStore::w_secure_get_current() => %s\n", esp_err_to_name(e)); return e; }
  std::string uk(l, '\0');
  e = nvs_get_str(h, kCurrKey, &uk[0], &l);
  nvs_close(h);
...
int SecureStore::get_current(Record* out) {
  ...
  if (err != ESP_OK) printf("err=%i\n", err);
  return err;
}

struct SUpdFieldArgs { const char* ns; std::string userkey, fname; std::vector<uint8_t> val; bool create; };
...
esp_err_t SecureStore::update_field(const std::string& userkey, const std::string& field_name, const std::vector<uint8_t>& value, bool create_record_if_missing) {
  SUpdFieldArgs a{ ns_, userkey, field_name, value, create_record_if_missing };
  return run_in_nvs_task(w_secure_update_field, &a);
}

struct SGetFieldArgs { const char* ns; std::string userkey, fname; std::vector<uint8_t>* out; };
...
esp_err_t SecureStore::get_field(const std::string& userkey, const std::string& field_name, std::vector<uint8_t>* out) {
  SGetFieldArgs a{ ns_, userkey, field_name, out };
  return run_in_nvs_task(w_secure_get_field, &a);
}

esp_err_t SecureStore::set_field_str(const std::string& userkey, const std::string& field_name, const std::string& value, bool create_record_if_missing) {
  return update_field(userkey, field_name, as_bytes(value), create_record_if_missing);
}

esp_err_t SecureStore::get_field_str(const std::string& userkey, const std::string& field_name, std::string* out) {
  if (!out) return ESP_ERR_INVALID_ARG;
  std::vector<uint8_t> v;
  esp_err_t e = get_field(userkey, field_name, &v);
  if (e != ESP_OK) return e;
...
struct SExportArgs { const char* ns; const char* path; };
...
struct SImportArgs { const char* ns; const char* path; bool overwrite; };
...
struct SFirstArgs { const char* ns; Record* out; };
static esp_err_t w_secure_get_first(void* pv) {
  ...
  if (nvs_get_str(h, kname.c_str(), nullptr, &len) != ESP_OK) { nvs_close(h); return ESP_ERR_NOT_FOUND; }
  std::string uk(len, '\0');
  if (nvs_get_str(h, kname.c_str(), &uk[0], &len) != ESP_OK) { nvs_close(h); return ESP_FAIL; }
