...
class TrackPlayer : bell::Task {
 public:
  ...
  // Callback types
  typedef std::function<void(std::shared_ptr<spotify::QueuedTrack>, State)>
  ...
  TrackPlayer(std::shared_ptr<spotify::Context> ctx,
              std::shared_ptr<spotify::TrackQueue> trackQueue,
              StateChangedCallback onStateChange, bool* track_repeat);
  ~TrackPlayer();
 ...
 private:
  ...
  std::vector<uint8_t> pcmBuffer = std::vector<uint8_t>(1024 * 4);
