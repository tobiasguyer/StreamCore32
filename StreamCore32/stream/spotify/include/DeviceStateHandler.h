...
class DeviceStateHandler : public bell::Task{
 public:
  ...
  DeviceStateHandler(std::shared_ptr<spotify::LoginBlob>, std::function<void(bool)>, std::function<void()>,
  ...
  void putDeviceState(player_proto_connect_PutStateReason member_type =
  ...
  void putPlayerState(player_proto_connect_PutStateReason member_type =
                          player_proto_connect_PutStateReason::player_proto_connect_PutStateReason_PLAYER_STATE_CHANGED);
  void handleConnectState();

  void sinkCommand(CommandType, CommandData data = {});

  void setPlayerState(std::shared_ptr<spotify::QueuedTrack> track, spotify::TrackPlayer::State state);
 ...
 private:
  ...
  std::pair<uint8_t*, std::vector<player_proto_connect_ProvidedTrack>*> queuePacket = {
