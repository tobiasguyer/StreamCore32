...
public:
  ...
  SpotifyStream(std::shared_ptr <AudioControl> _audioController, std::unique_ptr<StreamCoreFile> _creds, std::function<void(bool)> _onConnect)
    : StreamBase("Spotify", _audioController, 1024 * 20, 1, 1, 1), creds(std::move(_creds)), onConnect_(_onConnect) { // cannot run on PSRAM because of NVS
    ...
    feed_->state_callback = [this](uint8_t state) {
      ...
      case 2:
        [[fallthrough]];
      case 3:    if (onUiMessage_ && (state == 2 || state == 3)) {
        nlohmann::json j;
        j["type"] = "playback";
        j["src"] = "Spotify";
  ...
  void runTask() override {
    ...
    if (!isRunning.load()) {
      ...
      try {
        std::function<void(bool)> _onClose = [this](bool logout) { this->onClose(logout); onConnect_(false);};
