...
struct Context {
  struct ConfigState {
    ...
    std::function<uint16_t()> volume = []() { return 0xFFFF; };
  ...
  static std::shared_ptr<Context> createFromBlob(
    ...
    SPOTIFY_LOG(info, "Using device id %s", ctx->config.deviceId.c_str());
    ctx->config.deviceName = blob->getDeviceName();
    SPOTIFY_LOG(info, "Using device name %s", ctx->config.deviceName.c_str());
    ctx->config.authData = blob->authData;
    SPOTIFY_LOG(info, "Using auth data of size %d",
              (int)ctx->config.authData.size());
    ctx->config.username = blob->getUserName();
    SPOTIFY_LOG(info, "Using username %s", ctx->config.username.c_str());
    return ctx;
  }
};
}  // namespace spotify
