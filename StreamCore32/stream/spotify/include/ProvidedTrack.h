...
bool add_metadata(player_proto_connect_ProvidedTrack* t, const char* k,
                  const char* v) {
...
bool add_metadata_list(player_proto_connect_ProvidedTrack* t,
                       const MetadataEntry* src, size_t src_count) {
  if (!t || !src || src_count == 0)
    return true;
...
fail:
  ...
  if (q)
    t->metadata = (MetadataEntry*)q;

  return false;
}

bool add_metadata_list(
...
fail:
  ...
  if (old == 0) {
    ...
    if (q)
      t->metadata = static_cast<MetadataEntry*>(q);
  }
  return false;
}
bool add_metadata_from_json_object(player_proto_connect_ProvidedTrack* t,
