...
class ZeroconfAuthenticator {
 public:
  ...
  void registerMdnsService() {
    ...
    spec.endpoints.push_back({ZeroconfServiceManager::HttpMethod::GET,
                              "/spotify_info",
                              [this](mg_connection* c) -> std::string {
                                return this->blob->buildZeroconfInfo();
                              }});
    spec.endpoints.push_back(
        {ZeroconfServiceManager::HttpMethod::POST, "/spotify_info",
         [this](mg_connection* c) -> std::string {
           nlohmann::json obj;
    ...
    spec.endpoints.push_back({ZeroconfServiceManager::HttpMethod::GET, "/close",
