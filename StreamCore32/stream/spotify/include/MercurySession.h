...
class MercurySession : public bell::Task, public spotify::Session {
 public:
  MercurySession(std::shared_ptr<spotify::TimeProvider> timeProvider);
  ~MercurySession();
 ...
 private:
  ...
  std::deque<spotify::Packet> packetQueue;

  void runTask() override;
  std::unordered_map<uint16_t, ResponseCallback> callbacks;
