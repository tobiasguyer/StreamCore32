...
class TrackMetrics {
 public:
  TrackMetrics(std::shared_ptr<spotify::Context> ctx, uint64_t pos = 0);
...
class PlaybackMetrics {
 ...
 public:
  PlaybackMetrics(std::shared_ptr<spotify::Context> ctx) { this->ctx = ctx; }
  enum reason {
    TRACK_DONE,
    TRACK_ERROR,
    FORWARD_BTN,
  ...
  void uri2context(std::string uri);
  std::string context_uri, correlation_id, start_source, end_source;

  std::vector<uint8_t> sendEvent(spotify::QueuedTrack* track);
};
}  // namespace spotify