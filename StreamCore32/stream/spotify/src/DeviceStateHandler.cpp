...
void remove_tracks_by_provider(
    std::vector<player_proto_connect_ProvidedTrack>& v, const char* provider,
    size_t offset, bool is_provided_by = false) {
  if (offset >= v.size())
    return;
...
static inline char* pb_map_get_value(const Entry* md, std::size_t md_count,
                                     const char* key) {
  if (!md || !key)
    return nullptr;
...
static inline bool move_ContextTrack_to_ProvidedTrack(
    player_proto_ContextTrack* src, player_proto_connect_ProvidedTrack* dst) {
  if (!src || !dst)
    return false;
...
DeviceStateHandler::DeviceStateHandler(
    std::shared_ptr<spotify::LoginBlob> blob, std::function<void(bool)> onClose,
  ...
  auto connectStateSubscription = [this](MercurySession::Response res) {
    ...
    if (strstr(res.mercuryHeader.uri, "v1/devices/")) {
      putDeviceState(player_proto_connect_PutStateReason::
                         player_proto_connect_PutStateReason_SPIRC_NOTIFY);
      ...
      if (res.parts[0].size()) {
        ...
        pbDecode(newVolume, player_proto_connect_SetVolumeCommand_fields,
                 res.parts[0]);
      ...
      SC32_LOG(debug, "Unknown connect_state, uri : %s", res.mercuryHeader.uri);
  };
  ...
  auto socialConnectSubscription = [this](MercurySession::Response res) {
    ...
    changePlayerState:
      if (this->is_active) {
        ...
        this->putDeviceState(
            player_proto_connect_PutStateReason::
                player_proto_connect_PutStateReason_BECAME_INACTIVE);
  ...
  if (ctx->config.authData.size() > 0) {
    ...
    this->trackPlayer = std::make_shared<TrackPlayer>(
        ctx, trackQueue,
        [this](std::shared_ptr<spotify::QueuedTrack> track,
    ...
    device.device_info.capabilities.supported_types[1] =
        strdup("audio/episode");
    ...
    device.device_info.device_type = player_proto_connect_DeviceType::
        player_proto_connect_DeviceType_SPEAKER;
...
void DeviceStateHandler::setPlayerState(
    ...
    case spotify::TrackPlayer::State::STOPPED:
      if (needsToBeSkipped) {
        if (this->device.player_state.options.repeating_track &&
            state == spotify::TrackPlayer::State::STOPPED) {
          this->trackQueue->preloadedTracks[0]->requestedPosition = 0;
          this->trackQueue->preloadedTracks[0]->state =
              QueuedTrack::State::READY;
      ...
      if (!this->trackQueue->preloadedTracks.size()) {
        ...
        SC32_LOG(info, "preloadedTracks size: %d",
                 this->trackQueue->preloadedTracks.size());
      break;
    default:
      break;
  }
...
void DeviceStateHandler::reloadTrackList(void* data) {
  handler->ctx->playbackMetrics->uri2context(
      handler->playerContext->context_uri);
  if (data == NULL) {
  ...
  if (strcmp(handler->currentTracks[handler->offset - 1].uri,
    ...
    handler->ctx->playbackMetrics->end_reason =
        spotify::PlaybackMetrics::REMOTE;
    ...
    handler->device.player_state.restrictions =
        player_proto_Restrictions_init_zero;
...
void DeviceStateHandler::putDeviceState(
    player_proto_connect_PutStateReason put_state_reason) {
  ...
  player_proto_connect_PutStateRequest tempPutReq =
      player_proto_connect_PutStateRequest_init_zero;
  tempPutReq.has_device = true;
  tempPutReq.has_member_type = true;
  tempPutReq.member_type = player_proto_connect_MemberType::
      player_proto_connect_MemberType_CONNECT_STATE;
  ...
  auto player_proto_connect_PutStateRequest =
      pbEncode(player_proto_connect_PutStateRequest_fields, &tempPutReq);
  tempPutReq.device = player_proto_connect_Device_init_zero;
  pb_release(player_proto_connect_PutStateRequest_fields, &tempPutReq);
  auto parts =
      MercurySession::DataParts({player_proto_connect_PutStateRequest});
...
void DeviceStateHandler::putPlayerState(
    player_proto_connect_PutStateReason put_state_reason) {
  //std::scoped_lock lock(playerStateMutex);
  ...
  if (device.player_state.track.provider &&
    ...
    if (device.player_state.has_restrictions)
      pb_release(player_proto_Restrictions_fields,
                 &device.player_state.restrictions);
    pb_release(player_proto_connect_ContextIndex_fields,
    ...
    device.player_state.index = player_proto_connect_ContextIndex{
        true, device.player_state.track.page, true,
...
void DeviceStateHandler::skip(CommandType dir, bool notify) {
  if (dir == CommandType::SKIP_NEXT) {
    ...
    offset--;
    trackQueue->preloadedTracks.push_front(
        std::make_shared<spotify::QueuedTrack>(
  ...
  if (command != jsonResult.end()) {
    ...
    if (command->at("endpoint") == "transfer") {
      ...
      std::vector<uint8_t> bytes =
          base64ToBytes(s);  // UTF-8 bytes of the string

      pbDecode(transfer_state, player_proto_transfer_TransferState_fields,
               bytes);

      spotify::TrackReference::clearProvidedTracklist(&currentTracks);
      currentTracks = {};

      if (transfer_state.has_options)
        PB_MOVE_ASSIGN(player_proto_ContextPlayerOptions,
                       device.player_state.options, transfer_state.options);
      if (transfer_state.current_session.has_option_overrides) {
        if (transfer_state.current_session.option_overrides
                .has_repeating_context) {
          this->device.player_state.options.repeating_context =
              transfer_state.current_session.option_overrides.repeating_context;
          this->device.player_state.options.has_repeating_context = true;
        }
        if (transfer_state.current_session.option_overrides
                .has_repeating_track) {
          this->device.player_state.options.repeating_track =
              transfer_state.current_session.option_overrides.repeating_track;
          this->device.player_state.options.has_repeating_track = true;
        }
        if (transfer_state.current_session.option_overrides
                .has_shuffling_context) {
          this->device.player_state.options.shuffling_context =
              transfer_state.current_session.option_overrides.shuffling_context;
      ...
      if (pb_map_get_value<MetadataEntry>(
        ...
        if (this->device.player_state.options.context_enhancement_count) {
          for (int i = 0;
               i < this->device.player_state.options.context_enhancement_count;
               i++) {
            pb_release(
                MetadataEntry_fields,
                &this->device.player_state.options.context_enhancement[i]);
          }
        ...
        this->device.player_state.options.context_enhancement =
            (MetadataEntry*)calloc(1, sizeof(MetadataEntry));
      ...
      offsetFromStartInMillis =
          transfer_state.playback.position_as_of_timestamp;
      if (transfer_state.playback.has_timestamp)
        offsetFromStartInMillis +=
            (int64_t)this->ctx->timeProvider->getSyncedTimestamp() -
            transfer_state.playback.timestamp;
      this->device.player_state.has_is_playing = true;
      this->device.player_state.is_playing = true;
      char* temp = pb_map_get_value<MetadataEntry>(
          transfer_state.playback.current_track.metadata,
          transfer_state.playback.current_track.metadata_count,
          "interaction_id");
      if (temp != NULL) {
        metadata_map.push_back(
            std::make_pair("interaction_id", std::string(temp)));
      } else if (logging_params != command->end() &&
                 logging_params->find("interaction_ids") !=
                     logging_params->end()) {
        metadata_map.push_back(std::make_pair(
            "interaction_id",
            logging_params->at("interaction_ids")[0].get<std::string>()));
      }
      temp = pb_map_get_value<MetadataEntry>(
          transfer_state.playback.current_track.metadata,
          transfer_state.playback.current_track.metadata_count,
          "page_instance_id");
      if (temp != NULL) {
        metadata_map.push_back(
            std::make_pair("page_instance_id", std::string(temp)));
      } else if (logging_params != command->end() &&
                 logging_params->find("page_instance_ids") !=
                     logging_params->end()) {
        metadata_map.push_back(std::make_pair(
            "page_instance_id",
      ...
      player_proto_connect_ProvidedTrack track =
          player_proto_connect_ProvidedTrack_init_default;
      move_ContextTrack_to_ProvidedTrack(&transfer_state.playback.current_track,
                                         &track);
      ...
      player_proto_ContextPage* pages =
          transfer_state.current_session.context.pages;
      if (pages != NULL) {
        for (int i = 0; i < pages->tracks_count; i++) {
          if (strcmp(compare, pages->tracks[i].uri) == 0 ||
              strcmp(compare, pages->tracks[i].uid) == 0) {
            continue;
          }
      ...
      this->device.player_state.context_uri =
          transfer_state.current_session.context.uri;
      this->device.player_state.context_url =
          transfer_state.current_session.context.url;
      ...
      if (command->at("endpoint") == "play") {
        ...
        if (command->find("play_origin") != command->end()) {
          pb_release(player_proto_connect_PlayOrigin_fields,
                     &device.player_state.play_origin);
          device.player_state.play_origin =
              player_proto_connect_PlayOrigin_init_zero;
        ...
        if (metadata != command->at("context").end()) {
          if (this->device.player_state.options.context_enhancement_count) {
            for (int i = 0;
                 i <
                 this->device.player_state.options.context_enhancement_count;
                 i++) {
              pb_release(
                  MetadataEntry_fields,
                  &this->device.player_state.options.context_enhancement[i]);
          ...
          if (metadata->find("enhanced_context") != metadata->end() &&
              !metadata->at("enhanced_context").is_boolean()) {
            this->device.player_state.options.context_enhancement =
                (MetadataEntry*)calloc(1, sizeof(MetadataEntry));
            this->device.player_state.options.context_enhancement[0].key =
                strdup("context_enhancement");
          ...
          for (int i = 0; i < this->device.player_state.context_metadata_count;
               i++) {
            pb_release(MetadataEntry_fields,
                       &device.player_state.context_metadata[i]);
          }
          free(this->device.player_state.context_metadata);
          this->device.player_state.context_metadata = (MetadataEntry*)calloc(
              context_metadata_map.size(), sizeof(MetadataEntry));
          ...
          if (this->device.player_state.options.context_enhancement_count) {
            for (int i = 0;
                 ...
                 ++i) {
              pb_release(MetadataEntry_fields,
                         &device.player_state.options.context_enhancement[i]);
            }
            free(this->device.player_state.options.context_enhancement);
            this->device.player_state.options.context_enhancement = NULL;
            this->device.player_state.options.context_enhancement_count = 0;
        ...
        if (auto* url = this->device.player_state.context_url) {
          if (const char* colon = strchr(url, ':'))
            provider.assign(url, colon - url);
        } else if (strchr(this->device.player_state.context_uri, ':') !=
                   strrchr(this->device.player_state.context_uri, ':')) {
          provider = "context";
        }
        if (options->find("skip_to")->find("track_uri") !=
            options->find("skip_to")->end()) {
          player_proto_connect_ProvidedTrack track =
              player_proto_connect_ProvidedTrack_init_zero;
          ...
          currentTracks.push_back(track);
                   ...
                   command->at("context").at("pages")[0]["tracks"].size() >
                       playlist_offset) {
          auto json_track =
              command->at("context")["pages"][0]["tracks"].at(playlist_offset);
          player_proto_connect_ProvidedTrack track =
              player_proto_connect_ProvidedTrack_init_zero;
          track.original_index = playlist_offset;
          track.uri =
              PlayerContext::createStringReferenceIfFound(json_track, "uri");
          track.uid =
              PlayerContext::createStringReferenceIfFound(json_track, "uid");
        ...
        if (tracks != command->end()) {
          ...
          int jsonoffset = 0;
          while (jsonoffset < tracks->size() &&
                 trackoffset < currentTracks.size()) {
            if (!tracks->at(jsonoffset).contains("provider") ||
                tracks->at(jsonoffset).at("provider") == "queue") {
              if (tracks->at(jsonoffset)["uri"] ==
                  tracks->at(trackoffset).at("uri")) {
                trackoffset++;
              } else {
                //Add track to back of queue
                player_proto_connect_ProvidedTrack track = {};
                track.uri = strdup(tracks->at(jsonoffset)
                                       .at("uri")
                                       .get<std::string>()
                                       .c_str());
                track.provider = strdup("queue");
        ...
        if (this->device.player_state.options.repeating_context !=
          ...
          if (release) {
            for (int i = release; i < currentTracks.size(); i++)
              spotify::TrackReference::pbReleaseProvidedTrack(
                  &currentTracks[i]);
