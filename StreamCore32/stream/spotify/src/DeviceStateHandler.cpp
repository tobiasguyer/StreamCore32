...
/* Liefert md[i].value oder nullptr, wenn key nicht existiert. */
...
pb_map_get_value(const Entry* md, std::size_t md_count, const char* key)
{
    ...
    for (std::size_t i = 0; i < md_count; ++i) {
        ...
        if (k && std::strcmp(k, key) == 0) {
            return md[i].value; // Achtung: Eigentum bleibt bei der Message
        }
    }
    return nullptr;
}
                                   ...
                                   player_proto_connect_ProvidedTrack* dst)
{
    ...
    *dst = player_proto_connect_ProvidedTrack_init_zero;
    ...
    // 1a) Falls keine URI, aus gid eine TRACK-URI erzeugen
    if (!dst->uri || dst->uri[0] == '\0') {
        // vorhandene leere URI aufräumen (falls z.B. "")
        if (dst->uri) { free(dst->uri); dst->uri = NULL; }
        // Nur wenn GID vorhanden / nicht leer
        if (src->gid /* ggf. weitere Prüfung in pbArrayToVector */) {
    ...
    if (!dst->provider) {
        // best effort rollback (dst->uri gehört jetzt schon dst)
        return false;
    }

    // 3) Metadata übernehmen (move)
    dst->metadata = NULL;
    ...
    if (n > 0 && src->metadata) {
        ...
        if (!dst->metadata) {
            return false; // dst->provider/dst->uri bleiben Eigentum von dst
        }

        for (size_t i = 0; i < n; ++i) {
            // Pointer-Besitz übernehmen
        ...
        // Quell-Array-Hülle freigeben und nullen
        free(src->metadata);
        src->metadata = NULL;
        src->metadata_count = 0;
    }

    // 4) Quelle ist nun „entkernt“: pb_release(transfer_state) ist sicher.
    return true;
}
...
DeviceStateHandler::DeviceStateHandler(std::shared_ptr<spotify::LoginBlob> blob,
                                       ...
                                       : bell::Task("deviceStateHandler", 16 * 1024, 0, 1) {
  handler = this;
  ...
  this->ctx->config.volume = volume;
    ...
    auto socialConnectSubscription = [this](MercurySession::Response res) {
      ...
      changePlayerState:
        if (this->is_active) {
          ...
          SPOTIFY_LOG(debug, "Device changed");
    ...
    SPOTIFY_LOG(info, "Added social-connect supscription");

    ctx->session->setConnectedHandler([this]() {
      SPOTIFY_LOG(info, "Registered new device");
      this->putDeviceState(
  ...
  SPOTIFY_LOG(info, "Connecting to Spotify");
  this->ctx->session->connectWithRandomAp();
  SPOTIFY_LOG(info, "Connected to Spotify");
  ctx->config.authData = ctx->session->authenticate(blob);
  SPOTIFY_LOG(info, "Authenticated to Spotify blob size : %d",
            (int)ctx->config.authData.size());
  this->onTransfer = onTransfer;
  if (ctx->config.authData.size() > 0) {
    SPOTIFY_LOG(info, "Starting DeviceStateHandler");
    onLogin(blob->getUserName(), ctx->config.authData);
    SPOTIFY_LOG(info, "Called onLogin callback");
    ...
    SPOTIFY_LOG(info, "Started player");
    ...
    SPOTIFY_LOG(error, "Authentication failed, closing connection");
    this->disconnect();
...
DeviceStateHandler::~DeviceStateHandler() {
  ...
  SPOTIFY_LOG(info, "DeviceStateHandler destroyed");  
}
    ...
    case spotify::TrackPlayer::State::STOPPED:
      if(needsToBeSkipped){
        if (this->device.player_state.options.repeating_track && state == spotify::TrackPlayer::State::STOPPED) {
          ...
          SPOTIFY_LOG(info, "Skipping track");
          skip(CommandType::SKIP_NEXT, true);
          SPOTIFY_LOG(info, "Skipped track");
        }
      }
      needsToBeSkipped = true;
      SPOTIFY_LOG(info, "Stopping playback");
      if ((uint32_t)currentTracks.size() / 2 <= offset &&
          !this->resolvingContext) {
        this->resolvingContext.store(true);
        SPOTIFY_LOG(info, "Resolving tracklist");
        playerContext->resolveTracklist(metadata_map, reloadTrackList);
      }
      if(!this->trackQueue->preloadedTracks.size()){
        SPOTIFY_LOG(info, "No more tracks");
        sinkCommand(CommandType::DISC);
      }
      else SPOTIFY_LOG(info, "preloadedTracks size: %d", this->trackQueue->preloadedTracks.size());
      break;
    default:
      break;
  }
...
void DeviceStateHandler::reloadTrackList(void* data) {
  ...
  //SPOTIFY_LOG(info,"heap_memory_check-safe = %i",heap_caps_check_integrity_all(true));
}
...
void DeviceStateHandler::disconnect(bool logout) {
  ...
  SPOTIFY_LOG(debug, "Disconnected from session");
  if(isRunning)
  onClose(logout);
  SPOTIFY_LOG(debug, "DeviceStateHandler disconnected");
...
void DeviceStateHandler::parseCommand(std::vector<uint8_t>& data) {
  ...
  try {
    ...
    SPOTIFY_LOG(error, "Failed to parse command");
  ...
  if (command != jsonResult.end()) {
    if (command->find("endpoint") == command->end())
      return;
    SPOTIFY_LOG(debug, "Parsing new command, endpoint : %s",
    ...
    if (command->at("endpoint") == "transfer") {
      ...
      if(pb_map_get_value<MetadataEntry>(
            ...
            "enhanced_context")
          != NULL){
        SPOTIFY_LOG(debug, "Enhanced context");
...
// CONTEXT 
        ...
        if(options->find("skip_to")->find("track_uri") != options->find("skip_to")->end()) {
          ...
          track.uid = PlayerContext::createStringReferenceIfFound(
              json_track, "uid");
              SPOTIFY_LOG(info, "track uri: %s", track.uri);
          track.provider = strdup(provider.c_str());
          currentTracks.push_back(track);
        }
        offset = 0;
        SPOTIFY_LOG(info, "Tracklist reloaded");
        ...
        SPOTIFY_LOG(info, "%s", jsonResult.dump().c_str());
        ...
        if (this->device.player_state.options.repeating_context)
          ...
          SPOTIFY_LOG(error, "Unknown command: %s",
                    &command->at("endpoint").get<std::string>()[0]);
          SPOTIFY_LOG(debug, "data: %s", command->dump(2).c_str());
        }
      return;
    }
  }
