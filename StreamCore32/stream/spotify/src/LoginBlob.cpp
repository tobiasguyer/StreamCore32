...
#include "ConstantParameters.h"          // for brandName, spotify32, protoc...
#include "Logger.h"                      // for spotify32_LOG
...
std::string createDeviceId() {
  ...
  SPOTIFY_LOG(info, "Mac-address : %s", mac_string.c_str());
  std::vector<uint8_t> mac(mac_string.begin(), mac_string.end());
  return sha1_digest(mac);
}
...
LoginBlob::LoginBlob(std::string name, std::string username, std::vector<uint8_t> authData) {
  ...
  this->authType = AuthenticationType_AUTHENTICATION_STORED_SPOTIFY_CREDENTIALS;
...
std::vector<uint8_t> LoginBlob::decodeBlob(
  ...
  // Check checksum
  if (mac != checksum) {
    SPOTIFY_LOG(error, "Mac doesn't match! Mac: %s, Checksum: %s",
...
std::string LoginBlob::buildZeroconfInfo() {
  ...
  cJSON_AddStringToObject(json_obj, "version", spotify::protocolVersion);
  cJSON_AddStringToObject(json_obj, "libraryVersion", spotify::swVersion);
  cJSON_AddStringToObject(json_obj, "accountReq", "PREMIUM");
  cJSON_AddStringToObject(json_obj, "brandDisplayName", spotify::brandName);
  ...
  obj["status"] = 101;
  obj["statusString"] = "OK";
  obj["version"] = spotify::protocolVersion;
  obj["spotifyError"] = 0;
  obj["libraryVersion"] = spotify::swVersion;
  obj["accountReq"] = "PREMIUM";
  obj["brandDisplayName"] = spotify::brandName;
