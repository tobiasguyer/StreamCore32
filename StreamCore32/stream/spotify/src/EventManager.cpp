...
TrackMetrics::TrackMetrics(std::shared_ptr<spotify::Context> ctx,
                           uint64_t pos) {
  this->ctx = ctx;
...
std::vector<uint8_t> PlaybackMetrics::sendEvent(spotify::QueuedTrack* track) {
  std::string data;
  ...
  append(
      &msg,
      track->trackInfo
          .page_instance_id);  //page-instance-id // changing , if radio / autoplay empty
  append(
      &msg,
      track->trackInfo
          .interaction_id);  //interaction_id // stays still , if radio / autoplay capped
  append(&msg, "");
  append(&msg, "5003900000000146");
  append(&msg, "");
  append(&msg, track->trackInfo.decision_id);
  append(&msg, "");
  append(&msg, "0");  //?? usually 0
  append(&msg, "0");  //?? usually 0
  ...
  // Execute the request
  ctx->session->execute(MercurySession::RequestType::POST, requestUrl, nullptr,
                        parts);
#endif
  return msg;
}