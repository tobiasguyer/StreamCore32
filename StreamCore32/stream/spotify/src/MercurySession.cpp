...
MercurySession::MercurySession(std::shared_ptr<TimeProvider> timeProvider)
    : bell::Task("mercury_dispatcher", 16 * 1024, 3,
...
bool MercurySession::processPackets() {
  try {
    spotify::Packet packet = shanConn->recvPacket();
    ...
    SPOTIFY_LOG(error, "Error while receiving packet: %s", e.what());
    failAllPending();  // Fail all pending requests
    return false;
  } catch (const std::exception& e) {
    SPOTIFY_LOG(error, "Unexpected exception: %s", e.what());
    failAllPending();  // Fail all pending requests
    return false;
  } catch (...) {
    SPOTIFY_LOG(error, "Unknown error occurred while receiving packet.");
    failAllPending();  // Fail all pending requests
    return false;
  }
}
...
void MercurySession::reconnect() {
  while (isRunning) {
    try {
      ...
      SPOTIFY_LOG(info, "Reconnection successful");
      ...
      SPOTIFY_LOG(error, "Cannot reconnect, will retry in 5s");
      BELL_SLEEP_MS(1000);
...
bool MercurySession::triggerTimeout() {
  ...
  if (currentTimestamp - this->lastPingTimestamp > PING_TIMEOUT_MS) {
    SPOTIFY_LOG(debug, "Reconnection required, no ping received");
    return true;
  }
...
void MercurySession::disconnect() {
  SPOTIFY_LOG(info, "Disconnecting mercury session");
  isRunning.store(false);
  std::scoped_lock lock(this->isRunningMutex);
  conn->close();
}
...
void MercurySession::handlePacket() {
  if(!this->responseSemaphore->twait(200)) return;
  std::unique_lock<std::mutex> lock(queueMutex);
  if (!packetQueue.size())
    return;
  ...
  switch (static_cast<RequestType>(packet.command)) {
    case RequestType::COUNTRY_CODE_RESPONSE: {
      ...
      SPOTIFY_LOG(debug, "Received country code %s", this->countryCode.c_str());
      break;
    }
    ...
    case RequestType::UNSUB: {
      ...
      if (!response.fail) {
        ...
        std::function<void(Response)> callbackToExecute = nullptr;    
        auto it = this->callbacks.find(response.sequenceId);
        if (it != this->callbacks.end()) {
            callbackToExecute = std::move(it->second);  // Move the callback out of the map
            this->callbacks.erase(it);  // Remove the callback entry from the map
        }
        lock.unlock();
        if (callbackToExecute) {
          callbackToExecute(response);
        } else {
          SPOTIFY_LOG(debug, "Callback not found for sequence id %d", response.sequenceId);
          if(response.mercuryHeader.uri) SPOTIFY_LOG(debug, "Response URI: %s", response.mercuryHeader.uri);
          if(response.mercuryHeader.method) SPOTIFY_LOG(debug, "Response method: %s", response.mercuryHeader.method);
          SPOTIFY_LOG(debug, "Response status: %d", response.mercuryHeader.status_code);
          SPOTIFY_LOG(debug, "Response type: %d", packet.command);
          SPOTIFY_LOG(debug, "Response bodies:");
          for (const auto& part : response.parts) {
            SPOTIFY_LOG(debug, "Part size: %zu", part.size());
            for(const auto& byte : part) {
              printf("%02x", byte);
            } printf("\n");
          }
          if(!response.parts.size()) {
