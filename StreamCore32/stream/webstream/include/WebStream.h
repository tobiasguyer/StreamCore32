...
public:
  ...
  struct IcyHeaders { int metaInt = 0; std::string contentType; std::string stationName; std::string codec; uint32_t bitrateKbps = 0; uint32_t sampleRateHz = 0; uint8_t channels = 0; };
...
protected:
  void runTask() override {
    ...
    feed_->state_callback = [this](uint8_t s) { state = s; };
    while (isRunning_.load()) {
      ...
      else SC32_LOG(info, "Resolved to %s", (*resolved).c_str());
      wantRestart_.store(true);
      ...
      while (!wantStop_.load()) {
        ...
        if (ret == 0) {
          BELL_SLEEP_MS(10);
          wantRestart_.store(true); wantStop_.store(true); break;
        }
        uint16_t written = 0;
    ...
    while (state != 7) BELL_SLEEP_MS(10);
    isRunning_.store(false);
  }
...
private:
  std::unique_ptr<bell::HTTPClient::Response> open(const std::string& url, const std::string& station, uint32_t trackId) {
    ...
    for (auto& header : h) {
      ...
      if (lh == "content-type") {
        ...
        if (lc.find("audio/mpeg") != std::string::npos || lc.find("audio/mp3") != std::string::npos || lc.find("audio/x-mpeg") != std::string::npos) {
          H.codec = "Mp3";
        }
        else if (lc.find("audio/aac") != std::string::npos || lc.find("aacp") != std::string::npos || lc.find("audio/aacp") != std::string::npos || lc.find("audio/mp4") != std::string::npos || lc.find("application/aac") != std::string::npos) {
          H.codec = "AAC";
          BELL_LOG(debug, "webstream", "AAC");
        }
        else if (lc.find("audio/ogg") != std::string::npos || lc.find("application/ogg") != std::string::npos) {
          if (lc.find("opus") != std::string::npos) H.codec = "Opus";
          else if (lc.find("vorbis") != std::string::npos) H.codec = "Vorbis";
          else H.codec = "Ogg";
        }
        else if (lc.find("audio/wav") != std::string::npos || lc.find("audio/x-wav") != std::string::npos || lc.find("audio/l16") != std::string::npos) {
          H.codec = "Pcm";
        }
        else if (lc.find("audio/flac") != std::string::npos || lc.find("flac") != std::string::npos) {
          H.codec = "FLAC";
        }
        else H.codec = "unknown";
      }
      else if ((lh == "icy-name" || lh == "name") && !header.second.empty()) {
        H.stationName = header.second;
      }
      else if ((lh == "icy-br" || lh == "icy-bitrate" || lh == "br") && !header.second.empty()) {
        H.bitrateKbps = toInt(header.second);
        if (H.bitrateKbps > 320 && H.bitrateKbps < 2000000) H.bitrateKbps /= 1000;
      }
      else if ((lh == "icy-sr" || lh == "samplerate" || lh == "sr") && !header.second.empty()) {
        H.sampleRateHz = toInt(header.second);
      }
      else if ((lh == "icy-channels" || lh == "channels" || lh == "ch") && !header.second.empty()) {
        H.channels = static_cast<uint8_t>(toInt(header.second));
      }
      else if (lh == "icy-metaint") {
        H.metaInt = toInt(header.second);
      }
      else if (lh == "transfer-encoding" && header.second.find("chunked") != std::string::npos) {
        isChunked_ = true;
      }
    }
    if (H.stationName.empty()) H.stationName = station;
    SC32_LOG(info, "headers: %s %d %s", H.contentType.c_str(), H.metaInt, H.stationName.c_str());
  ...
  int read(bell::HTTPClient::Response* stream, uint8_t* buffer, size_t chunk_size, size_t trackId) {
    ...
    if (bytesUntilMeta == 0) {
      ...
      int r;
      if (!isChunked_) {
        if (stream->read(&Lbyte, 1) != 1) return -1;
      }
      else {
      ...
      if (metaLen > 0)
      {
        ...
        if (!isChunked_) {
          ...
          int m = readChunkedBody(stream,
            reinterpret_cast<uint8_t*>(&meta[0]),
            (size_t)metaLen);
          if (m != metaLen) {
  ...
  bool streamOnce(const std::string& url, const std::string& station, uint32_t trackId) {
    ...
    auto h = resp->headers();
    for (auto& header : h) {
      BELL_LOG(debug, "webstream", "Checking header: %s: %s", header.first.c_str(), header.second.c_str());
    }
  ...
  static std::string parseStreamTitle(const std::string& meta) {
    ...
    while (v < m.size() && (m[v] == ' ' || m[v] == '\t')) ++v;
    if (v >= m.size()) return {};

    // find end of value at first ';' (or end of string)
    size_t end = m.find(';', v);
    if (end == std::string::npos) end = m.size();
  ...
  static bool readLine(HTTPClient::Response* is, std::string& out) {
    ...
    char c;
    while (true) {
      if (!is->read(reinterpret_cast<uint8_t*>(&c), 1)) return false;
      if (c == '\r') {
        // expect \n
        if (!is->read(reinterpret_cast<uint8_t*>(&c), 1)) return false;
        return true;
  ...
  static size_t parseHexSize(const std::string& s) {
    size_t val = 0;
    for (char c : s) {
      if (c == ';' || c == ' ' || c == '\t') break; // ignore extensions
      int v = -1;
      if (c >= '0' && c <= '9') v = c - '0';
