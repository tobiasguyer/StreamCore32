...
public:
  ...
  : bell::Task("MetaPoller", 4096 * 4, 0, 1, false), emit_(std::move(e)), err_(std::move(er)) {}
  ~MetaPoller() {
    while(isRunning_.load()) BELL_SLEEP_MS(100);
  }
  void arm(const std::string& origin, const std::string& station, const Spec& spec) {
    std::lock_guard<std::mutex> lk(mu_);
    origin_ = origin; station_ = station; spec_ = spec; active_.store(true);
  }
...
private:
  ...
  std::unique_ptr<bell::HTTPClient::Response> httpGetSimple(const std::string& url);
  ...
  static std::string parseShoutcast7(const std::string& body);
