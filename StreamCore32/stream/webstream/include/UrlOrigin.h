...
inline std::string schemeOf(const std::string& origin){ auto p=origin.find("://"); return (p==std::string::npos)?std::string("https"):origin.substr(0,p); }
inline std::string hostOf(const std::string& origin){ auto p=origin.find("://"); size_t start=(p==std::string::npos)?0:p+3; size_t slash=origin.find('/', start); return origin.substr(start,(slash==std::string::npos)?std::string::npos:slash-start); }
...
static std::vector<std::string> genOriginVariants(const std::string& origin) {
  ...
  auto add = [&](const std::string& host) {
    ...
    if (std::find(out.begin(), out.end(), u) == out.end()) out.push_back(std::move(u));
  };
  ...
  if (firstDot != std::string::npos && lastDot != std::string::npos && firstDot < lastDot) {
    ...
    if (!first.empty() && !tld.empty()) add(first + "." + tld);
  }
