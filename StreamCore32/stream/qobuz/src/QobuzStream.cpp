...
void QobuzStream::runTask() {
  ...
  std::function<std::unique_ptr<bell::HTTPClient::Response>(
      ...
      bool sign)>
      onQobuzGet =
          [this](const std::string& object, const std::string& action,
                 const std::vector<std::pair<std::string, std::string>>& params,
                 bool sign) -> std::unique_ptr<bell::HTTPClient::Response> {
    uint64_t now_ms = timesync::now_ms();
    if ((uint64_t)cfg_.XsessionId.expiresAt < now_ms)
      startSession();
    ...
    if (sign)
      return qobuzGet(cfg_.api_base, object, action, headers, params,
  ...
  std::function<std::unique_ptr<bell::HTTPClient::Response>(
    ...
    if (body.empty()) {
      if (sign)
        return qobuzPost(cfg_.api_base, object, action, headers, "", params,
                         timesync::now_s_text(6), cfg_.appSecret);
      else
        return qobuzPost(cfg_.api_base, object, action, headers, "", params);
    }
  ...
  wsManager->onAuth([this]() { WSRegisterController(); });
  wsManager->onPayload(
      [this](std::vector<uint8_t> data) { WSDecodePayload(std::move(data)); });
  wsManager->startTask();
  queue_->startTask();
...
bool QobuzStream::login() {
  ...
  const std::string url = cfg_.api_base + "/user/login?" +
                          build_query({{"email", cfg_.email},
                                       {"password", cfg_.password},
                                       {"app_id", cfg_.appId}});
  std::string msg = "extra=partner";

  std::unique_ptr<bell::HTTPClient::Response> qobuzResponse =
      std::make_unique<bell::HTTPClient::Response>();

  qobuzResponse->post(url, {}, std::vector<uint8_t>(msg.begin(), msg.end()));
  if (qobuzResponse->status() != 200)
    return false;
  ...
  if (!json.is_discarded()) {
    ...
    SC32_LOG(info,
             "Qobuz login finished with app_id: %s and user_auth_token: %s",
             cfg_.appId.c_str(), cfg_.userAuthToken.c_str());
  }
...
bool QobuzStream::refreshApiToken() {
  ...
  auto resp =
      qobuzPost(cfg_.api_base, "qws", "refreshToken", headers, "jwt=jwt_api");
  if (resp->status() == 200) {
...
std::unique_ptr<bell::HTTPClient::Response> QobuzStream::qobuzGet(
  ...
  if (params.size()) {
    ...
    if (request_ts.size())
      url += "&request_ts=" + request_ts;
    if (app_secret.size()) {
      const std::string sig =
          qobuz::md5_sig(object, action, params, request_ts, app_secret);
      url += "&request_sig=" + sig;
    }
  }
  std::unique_ptr<bell::HTTPClient::Response> qobuzResponse =
...
WSToken QobuzStream::getWSToken() {
  ...
  if (cfg_.userAuthToken.empty() && token.jwt.empty())
    return token;
  HTTPClient::Headers headers = {
      {"Referer", "https://play.qobuz.com/"},
      {"Content-Type", "application/x-www-form-urlencoded"},
...
bool QobuzStream::startSession() {
  if (cfg_.userAuthToken.empty() && cfg_.api_token.token.empty())
    return false;
  ...
  const std::string sig =
      qobuz::md5_sig("session", endpoint, params, ts_text, app_secret);

  const std::string body = qobuz::build_query(params) +
                           "&request_ts=" + ts_text + "&request_sig=" + sig;
  HTTPClient::Headers headers = {
      {"Referer", "https://play.qobuz.com/"},
      {"Content-Type", "application/x-www-form-urlencoded"},
      {"Origin", "https://play.qobuz.com"},
  ...
  if (resp->status() != 200) {
    ...
    if (!j.is_discarded()) {
      ...
      SC32_LOG(info, "Qobuz start: expires_at: %llu",
               cfg_.XsessionId.expiresAt);
      cfg_.XsessionId.expiresAt *= 1000;
      SC32_LOG(info, "Qobuz start: session_id: %s",
               cfg_.XsessionId.token.c_str());
      SC32_LOG(info, "Qobuz start: expires_at: %llu",
               cfg_.XsessionId.expiresAt);
      cfg_.infos = j["infos"];
...
bool QobuzStream::open(const std::string& trackId) {
  if (cfg_.appSecret.empty())
    return false;
  timesync::wait_until_valid(8000);
  const std::string ts_text = timesync::now_s_text(6);

  const std::vector<std::pair<std::string, std::string>> params = {
      {"format_id", "5"}, {"intent", "stream"}, {"track_id", trackId}};
  ...
  auto resp = qobuzGet(cfg_.api_base, "track", "getFileUrl", headers, params,
                       ts_text, cfg_.appSecret);
  if (resp->status() != 200) {
...
void QobuzStream::WSRegisterController() {
  ...
  device.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_CTRL_SRVR_JOIN_SESSION;
  ...
  device.ctrlSrvrJoinSession.deviceInfo.friendlyName =
      strdup(cfg_.name.c_str());
  device.ctrlSrvrJoinSession.deviceInfo.has_type = true;
  device.ctrlSrvrJoinSession.deviceInfo.type =
      qconnect_DeviceType_DEVICE_TYPE_SPEAKER;
  ...
  device.ctrlSrvrJoinSession.deviceInfo.softwareVersion =
      strndup("sc32-1.0.0", strlen("sc32-1.0.0"));  //"sc32-1.0.0";
...
void QobuzStream::WSDecodeMessage(_qconnect_QConnectMessage* data) {
  switch (data->messageType) {
    case qconnect_QConnectMessageType_MESSAGE_TYPE_ERROR:
      ...
      if (strcmp(data->error.message,
                 "Current track not found in queue nor autoplay") == 0) {
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_ERROR_MESSAGE: {
      if (!data->has_srvrCtrlQueueErrorMessage)
        break;
      SC32_LOG(error, "Queue error message received");
      SC32_LOG(error, "Error code: %s",
               data->srvrCtrlQueueErrorMessage.error.code);
      SC32_LOG(error, "Error description: %s",
               data->srvrCtrlQueueErrorMessage.error.message);
      if (strcmp(data->srvrCtrlQueueErrorMessage.error.message,
                 "Queue version mismatch") == 0) {
        queue_->queueuState.queueVersion =
            data->srvrCtrlQueueErrorMessage.queueVersion;
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_ADD_RENDERER: {
      if (!data->has_srvrCtrlAddRenderer)
        break;
      if (std::equal(data->srvrCtrlAddRenderer.renderer.deviceUuid->bytes,
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_SESSION_STATE: {
      if (!data->has_srvrCtrlSessionState)
        break;
      queue_->queueuState.queueVersion =
          data->srvrCtrlSessionState.queueVersion;
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_ACTIVE_RENDERER_CHANGED: {
      if (!data->has_srvrCtrlActiveRendererChanged)
        break;
      SC32_LOG(info, "Active renderer changed to %llu",
               data->srvrCtrlActiveRendererChanged.rendererId);
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_STATE: {
      if (!data->has_srvrCtrlQueueState)
        break;
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_TRACKS_LOADED: {
      if (!data->has_srvrCtrlQueueTracksLoaded)
        break;
      SC32_LOG(info, "Queue tracks loaded - Queue version %llu/%llu",
               data->srvrCtrlQueueTracksLoaded.queueVersion.major,
               data->srvrCtrlQueueTracksLoaded.queueVersion.minor);
      queue_->deleteQobuzTracks();
      queue_->queueuState.queueVersion =
          data->srvrCtrlQueueTracksLoaded.queueVersion;
      WSAskForQueueState();
      WSAskForRendererState();
      queue_->addQobuzTracks(data->srvrCtrlQueueTracksLoaded.tracks,
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_TRACKS_INSERTED: {
      if (!data->has_srvrCtrlQueueTracksInserted)
        break;
      if (data->srvrCtrlQueueTracksInserted.autoplayReset) {
        queue_->deleteAutoplayTracks();
      }
      size_t insertIndex;
      queue_->position(insertIndex,
                       data->srvrCtrlQueueTracksInserted.insertAfter);
      queue_->addQobuzTracks(data->srvrCtrlQueueTracksInserted.tracks,
                             data->srvrCtrlQueueTracksInserted.tracks_count,
                             insertIndex,
                             data->srvrCtrlQueueTracksInserted.contextUuid);
      queue_->addShuffleIndexes(data->srvrCtrlQueueTracksInserted.tracks_count,
                                insertIndex);
      break;
    }
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_TRACKS_ADDED: {
      if (!data->has_srvrCtrlQueueTracksAdded)
        break;
      if (data->srvrCtrlQueueTracksAdded.autoplayReset) {
        queue_->deleteAutoplayTracks();
      }
      queue_->addQobuzTracks(data->srvrCtrlQueueTracksAdded.tracks,
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_TRACKS_REMOVED: {
      if (!data->has_srvrCtrlQueueTracksRemoved)
        break;
      queue_->deleteQobuzTracks(
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_AUTOPLAY_TRACKS_LOADED: {
      if (!data->has_srvrCtrlAutoplayTracksLoaded)
        break;
      ...
      queue_->deleteAutoplayTracks();
      queue_->addQobuzTracks(data->srvrCtrlAutoplayTracksLoaded.tracks,
                             data->srvrCtrlAutoplayTracksLoaded.tracks_count,
                             std::nullopt,
                             data->srvrCtrlAutoplayTracksLoaded.contextUuid);
      break;
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_AUTOPLAY_TRACKS_REMOVED: {
      if (!data->has_srvrCtrlAutoplayTracksRemoved)
        break;
      queue_->deleteQobuzTracks(
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_RENDERER_STATE_UPDATED: {
      if (!data->has_srvrCtrlRendererStateUpdated)
        break;
      if (data->srvrCtrlRendererStateUpdated.rendererId == cfg_.rendererId) {
        if (!player_->isRunning()) {
          queue_->setIndex(
              data->srvrCtrlRendererStateUpdated.state.currentQueueIndex);
          queue_->setStartAt(
              data->srvrCtrlRendererStateUpdated.state.currentPosition.value);
          this->player_->startTask();
        }
      ...
      if (player_->isRunning() || isActive_.load()) {
        if (state->has_currentPosition && !state->has_currentQueueItem) {
          ...
          if (currentTrack &&
              ...
              player_->currentTrackValueMs() >=
                  3000) {  //if more than 3s just restart
            player_->requestSkipTo(0);
            break;
          } else if (currentTrack && currentTrack->index ==
                                         state->currentQueueItem.queueItemId) {
            SC32_LOG(info, "current track %lu",
                     state->currentQueueItem.queueItemId);
          ...
          player_->player_state.playingState =
              qconnect_PlayingState_PLAYING_STATE_PAUSED;
        } else if (state->playingState ==
    ...
    case qconnect_QConnectMessageType_MESSAGE_TYPE_SRVR_CTRL_QUEUE_VERSION_CHANGED: {
      if (!data->has_srvrCtrlQueueVersionChanged)
        break;
      queue_->queueuState.queueVersion =
          data->srvrCtrlQueueVersionChanged.queueVersion;
      break;
    }
    default:
...
void QobuzStream::WSSetRendererActive() {
  ...
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_CTRL_SRVR_SET_ACTIVE_RENDERER;
...
void QobuzStream::WSAskForQueueState() {
  qconnect_QConnectMessage msg = qconnect_QConnectMessage_init_default;
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_CTRL_SRVR_ASK_FOR_QUEUE_STATE;
  msg.has_messageType = true;
  ...
  msg.ctrlSrvrAskForQueueState.queueUuid = static_cast<pb_bytes_array_t*>(
      malloc(PB_BYTES_ARRAY_T_ALLOCSIZE(16)));  //(&cfg_.queue_uuid);
  msg.ctrlSrvrAskForQueueState.queueUuid->size = 16;
  memcpy(msg.ctrlSrvrAskForQueueState.queueUuid->bytes, cfg_.session_id.raw,
         16);
  this->encodeBatches(&msg, 1);
}
void QobuzStream::WSAskForRendererState() {
  qconnect_QConnectMessage msg = qconnect_QConnectMessage_init_default;
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_CTRL_SRVR_ASK_FOR_RENDERER_STATE;
  msg.has_messageType = true;
...
void QobuzStream::WSSetRendererVolume() {
  qconnect_QConnectMessage msg = qconnect_QConnectMessage_init_default;
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_RNDR_SRVR_VOLUME_CHANGED;
  msg.has_messageType = true;
