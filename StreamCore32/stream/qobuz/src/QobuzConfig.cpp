...
static void writeHttp11Get(bell::SocketStream& s, const bell::URLParser& u) {
    ...
    req += "Host: " + u.host + "\r\n";
    req += "User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/124 Safari/537.36\r\n";
...
static bool parseHeaders(const std::string& headers,
                         int& status,
...
static bool readHttpResponse(bell::SocketStream& s,
                             int& status,
                             std::map<std::string,std::string>& headers,
                             std::vector<uint8_t>& raw) {
    // Read header block
    std::string hdrs; hdrs.reserve(2048);
    while (s.rdbuf()->sgetc() != std::char_traits<char>::eof()) {
        char c = (char)s.rdbuf()->sbumpc();
        hdrs.push_back(c);
        size_t n = hdrs.size();
        if (n >= 4 && hdrs[n-4]=='\r' && hdrs[n-3]=='\n' && hdrs[n-2]=='\r' && hdrs[n-1]=='\n') break;
        if (hdrs.size() > 64*1024) break; // guard
    }
    if (!parseHeaders(hdrs, status, headers)) return false;

    // Body to EOF
    char buf[1024];
    while (s.rdbuf()->sgetc() != std::char_traits<char>::eof()) {
        std::streamsize n = s.readsome(buf, sizeof(buf));
        if (n > 0) raw.insert(raw.end(), (uint8_t*)buf, (uint8_t*)buf + n);
        else break;
    }
    return true;
}
...
static std::string unescapeJSON(const std::string& in){
  ...
  for(size_t i=0;i<in.size();++i){
    ...
    if(c=='\\' && i+1<in.size()){
      char n=in[++i];
      switch(n){ case '"':out.push_back('"');break; case '\\':out.push_back('\\');break; case '/':out.push_back('/');break;
        ...
        case 'u': { if(i+4<in.size()){ int v= (hexVal(in[i+1])<<12)|(hexVal(in[i+2])<<8)|(hexVal(in[i+3])<<4)|(hexVal(in[i+4])); if(v>=0){ out.push_back((char)(v&0x7F)); i+=4; } } break; }
...
void scan_app_id(const std::string& hay, std::string& app_id) {
  ...
  BELL_LOG(info, "WEBSECRET", "found appId at %d", p);
  p += std::strlen(key);
  // next 9 digits
  std::string id;
  while (p < hay.size() && std::isdigit((unsigned char)hay[p]) && id.size() < 9) {
    id.push_back(hay[p++]);
  }
  if (id.size() == 9) app_id = id;
  BELL_LOG(info, "WEBSECRET", "found appId %s", app_id.c_str());
}
struct Seed { std::string tz_cap; std::string seed; };

void scan_seeds(const std::string& hay, std::map<std::string,std::string>& seeds) {
  // pattern:  x.initialSeed("SEED",window.utimezone.<tz>)
  const char* needle = ".initialSeed(\"";
  size_t i = 0;
  while ((i = hay.find(needle, i)) != std::string::npos) {
    BELL_LOG(info, "WEBSECRET", "found initialSeed at %d", i);
    i += std::strlen(needle);
    size_t q = hay.find('"', i);
    ...
    t += std::strlen(tzp);
    size_t e = t;
    while (e < hay.size() && std::isalpha((unsigned char)hay[e])) ++e;
    std::string tz = hay.substr(t, e - t); // e.g., "europe"
    capitalize_first(tz); // "Europe"
    seeds.emplace(tz, seed);
    BELL_LOG(info, "WEBSECRET", "seed for %s: %s", tz.c_str(), seed.c_str());
    i = e;
  }
}

void scan_info_extras_and_derive(const std::string& hay,
                                 ...
                                 std::map<std::string,std::string>& tz_secret)
{
  for (const auto& s : seeds) {
    ...
    std::string anchor = "/";       // any prefix before slash is fine; we only check suffix
    anchor += s.first;  // "Europe"
    size_t scan = 0;
    while ((scan = hay.find(anchor, scan)) != std::string::npos) {
      // grab info="...", extras="..."
      size_t info_k = hay.find("info:", scan);
      if(info_k != std::string::npos) BELL_LOG(info, "WEBSECRET", "found info %s at %d", hay.c_str(), scan); 
      size_t extras_k = hay.find("extras:", scan);
      if(extras_k != std::string::npos) BELL_LOG(info, "WEBSECRET", "found extra %s at %d", hay.c_str(), scan); 
      if (info_k == std::string::npos || extras_k == std::string::npos) break;
      ...
      if (info_end == std::string::npos || extras_end == std::string::npos) {
        ...
        if (info_end == std::string::npos || extras_end == std::string::npos) break;
      }
...
static void streamScanForQobuzSecrets(bell::SocketStream& s, QobuzConfig::ClientAppSecrets& web_secrets) {
  static const size_t WINSZ = 2048;   // sliding window size
  static const size_t CHUNK = 512;    // read size
  std::string win; win.reserve(WINSZ*2);
  ...
  while (s.rdbuf()->sgetc() != std::char_traits<char>::eof()) {
    ...
    if (win.size() > WINSZ*2) win.erase(0, win.size() - WINSZ); // keep tail
    if (web_secrets.id.empty()) scan_app_id(win, web_secrets.id);
...
void scanBundleFullStream(const std::string& jsUrl, QobuzConfig::ClientAppSecrets& out){
    ...
    // skip headers
    std::string hdrs; hdrs.reserve(1024);
    while (s.rdbuf()->sgetc()!=std::char_traits<char>::eof()){
        char c = (char)s.rdbuf()->sbumpc();
        hdrs.push_back(c);
        size_t n=hdrs.size();
        if(n>=4 && hdrs[n-4]=='\r'&&hdrs[n-3]=='\n'&&hdrs[n-2]=='\r'&&hdrs[n-1]=='\n') break;
        if (hdrs.size()>64*1024) break;
    }
...
static bool dechunk_http_body(const std::vector<uint8_t>& in, std::vector<uint8_t>& out) {
  ...
  while (i < n) {
    ...
    if (sz == 0) {
      ...
      if (!read_line(trailer)) return true;
      while (!trailer.empty()) {
        if (!read_line(trailer)) break;
      }
      return true;
    }
    if (i + (size_t)sz > n) return false;
...
static std::string absolutize(const std::string& base, const std::string& ref) {
    ...
    std::string hostport = b.host + ((b.port!=80 && b.port!=443)?(":"+std::to_string(b.port)):"");
    if (!ref.empty() && ref[0]=='/') return b.schema + "://" + hostport + ref;
    // relative
...
struct HttpResult { int status=0; std::string body; std::map<std::string,std::string> headers; };
static std::vector<std::string> extractScriptSrcs(const std::string& html) {
    ...
    auto push_if_js = [&](const std::string& url){
        ...
        if (url.find(".js") != std::string::npos) out.push_back(url);
    };
    ...
    while ((p = html.find("<script", p)) != std::string::npos) {
        ...
        if (s != std::string::npos && s < tagEnd) {
            ...
            if (s < html.size() && (html[s] == '"' || html[s] == '\'')) {
                ...
                if (e != std::string::npos) push_if_js(html.substr(s, e - s));
            }
        }
    ...
    while ((p = html.find("<link", p)) != std::string::npos) {
        ...
        auto low = frag; for (auto& c : low) c = (char)tolower((unsigned char)c);
        if (low.find("rel=\"preload\"") == std::string::npos &&
            low.find("rel='preload'") == std::string::npos) { p = tagEnd + 1; continue; }
        if (low.find("as=\"script\"") == std::string::npos &&
            low.find("as='script'") == std::string::npos) { p = tagEnd + 1; continue; }
        ...
        if (h != std::string::npos) {
            ...
            if (h < frag.size() && (frag[h] == '"' || frag[h] == '\'')) {
                ...
                if (e != std::string::npos) push_if_js(frag.substr(h, e - h));
            }
        }
        p = tagEnd + 1;
    }
...
static bool isPlayerAsset(const std::string& base, const std::string& url) {
    ...
    if (u.host != "play.qobuz.com") return false;
    // likely bundles live here
    if (u.path.find("/resources/") != std::string::npos) return true;
    if (u.path.find("/_next/") != std::string::npos) return true; // future-proof
    if (u.path.rfind(".js") != std::string::npos) return true;    // fallback
    return false;
}

static std::vector<std::string> extractPlayerScriptSrcs(const std::string& html, const std::string& baseUrl) {
    std::vector<std::string> all = extractScriptSrcs(html); // your existing function
    std::vector<std::string> out;
    out.reserve(all.size());
    for (auto& s : all) {
        if (isPlayerAsset(baseUrl, s)) out.push_back(absolutize(baseUrl, s));
    }
    // de-dup
...
static HttpResult bellGet_follow(const std::string& startUrl, int max_redirs=3) {
    ...
    for (int hop=0; hop<=max_redirs; ++hop) {
        ...
        bell::SocketStream s;
        if (s.open(u.host, u.port, isTLS) != 0) {                   // SocketStream picks TLS vs TCP via bool flag 
        ...
        if (status>=300 && status<400) {
            ...
            if (it!=hdrs.end()) { url = absolutize(url, it->second); continue; }
        }

        HttpResult r; r.status = status; r.headers = std::move(hdrs);
        auto te = r.headers.find("transfer-encoding");
        if (te!=r.headers.end() && te->second.find("chunked")!=std::string::npos) {
            std::vector<uint8_t> out;
            if (!dechunk_http_body(raw, out)) throw std::runtime_error("Chunked decode error");
...
void tryQobuzFromBundlesBounded(const std::string& entryUrl, QobuzConfig::ClientAppSecrets& secrets) {
    auto page = bellGet_follow(entryUrl);          // your robust GET+redirects, JSON headers
    auto scripts = extractPlayerScriptSrcs(page.body, entryUrl); // <— filter to play.qobuz.com
    ...
    for (size_t i=0; i<limit; ++i) {
        ...
        BELL_LOG(info, "qobuz", "wont scan JS[%u/%u]: %s", (unsigned)i+1, (unsigned)limit, jsUrl.c_str());
        BELL_SLEEP_MS(1);
        continue;
        }
        BELL_LOG(info, "qobuz", "scan JS[%u/%u]: %s", (unsigned)i+1, (unsigned)limit, jsUrl.c_str());
        BELL_SLEEP_MS(1);
...
QobuzConfig::ClientAppSecrets QobuzConfig::FetchClientAppSecrets()
{
    ...
    if (opts_.url.find("qobuz.com") != std::string::npos) {
        ...
        static const char* qobuzEntrypoints[] = {
        ...
        for (auto ep : qobuzEntrypoints) {
          try {
            ...
            if(!s.secrets.empty()) {
              BELL_LOG(info, "qobuz", "found %i secrets and %i api keys", s.secrets.size(), s.api_keys.size());
              for(auto& k : s.api_keys) BELL_LOG(info, "qobuz", "api key: %s", k.c_str());
              for(auto& k : s.secrets) BELL_LOG(info, "qobuz", "app id: %.8s..., secret: %.8s...", s.id.c_str(), k.second.c_str()); // BELL_LOG(info, "qobuz", "app id: %s", k.id.c_str());
              return s;
          }
          } catch (const std::bad_alloc&) {
            BELL_LOG(error, "qobuz", "OOM while scanning bundles. Lower slice sizes.");
