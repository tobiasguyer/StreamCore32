    ...
    std::map<std::string, std::string>& tz_secret)
  {
    for (const auto& s : seeds) {
      ...
      while ((scan = hay.find(anchor, scan)) != std::string::npos) {
        ...
        if (info_k != std::string::npos) SC32_LOG(info, "found info %s at %d", hay.c_str(), scan);
        size_t extras_k = hay.find("extras:", scan);
        if (extras_k != std::string::npos) SC32_LOG(info, "found extra %s at %d", hay.c_str(), scan);
  ...
  void tryQobuzFromBundlesBounded(const std::string& entryUrl, QobuzConfig::ClientAppSecrets& secrets) {
    ...
    for (size_t i = 0; i < limit; ++i) {
      ...
      if (u.host != "play.qobuz.com") {
        SC32_LOG(info, "wont scan JS[%u/%u]: %s", (unsigned)i + 1, (unsigned)limit, jsUrl.c_str());
        BELL_SLEEP_MS(1);
        continue;
      }
      SC32_LOG(info, "scan JS[%u/%u]: %s", (unsigned)i + 1, (unsigned)limit, jsUrl.c_str());
      BELL_SLEEP_MS(1);
  ...
  if (opts_.url.find("qobuz.com") != std::string::npos) {
    ...
    for (auto ep : qobuzEntrypoints) {
      try {
        ...
        if (!s.secrets.empty()) {
          SC32_LOG(info, "found %i secrets and %i api keys", s.secrets.size(), s.api_keys.size());
          for (auto& k : s.api_keys) SC32_LOG(info, "api key: %s", k.c_str());
          for (auto& k : s.secrets) SC32_LOG(info, "app id: %.8s..., secret: %.8s...", s.id.c_str(), k.second.c_str()); // SC32_LOG(info, "app id: %s", k.id.c_str());
          return s;
        }
      }
