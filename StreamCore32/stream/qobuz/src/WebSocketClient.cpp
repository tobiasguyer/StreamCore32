...
static std::string b64(const uint8_t* d, size_t n) {
  ...
  while (i + 2 < n) {
    ...
    o.push_back(B[(v >> 18) & 63]);
    o.push_back(B[(v >> 12) & 63]);
    o.push_back(B[(v >> 6) & 63]);
    o.push_back(B[v & 63]);
    i += 3;
  }
  if (i + 1 == n) {
...
std::string WebSocketClient::genSecKey() {
  ...
  for (int i = 0; i < 16; i += 4) {
    uint32_t w = streamcore::esp_random_engine()();
    std::memcpy(r + i, &w, std::min(4, 16 - i));
  }
  return b64(r, 16);
}

bool WebSocketClient::connect(const std::string& url, const std::string& origin,
                              const std::vector<std::string>& subs) {
  // 1st attempt: with provided subprotocols ({"qws"} by default)
  if (handshake(url, origin, subs))
    return true;

  // 2nd attempt: no subprotocols at all
  SC32_LOG(error, "handshake retry without subprotocols");
  if (handshake(url, origin, {}))
    return true;
...
bool WebSocketClient::handshake(const std::string& url,
  ...
  if (u.schema != "wss") {
    SC32_LOG(error, "non-wss schema: %s", u.schema.c_str());
    return false;
  }
  ...
  if (!tls_->isOpen()) {
    SC32_LOG(error, "TLS open failed");
    return false;
  }
  ...
  if (tls_->write(reinterpret_cast<const uint8_t*>(req.data()), req.size()) <=
      0) {
  ...
  while (waited_ms < 6000) {
    ...
    if (r == 1) {
      ...
      if (hdr.size() >= 4 && hdr.compare(hdr.size() - 4, 4, "\r\n\r\n") == 0)
        break;
      if (hdr.size() > 16384) {
        SC32_LOG(error, "header too large");
        return false;
      }
    ...
    if (r == 0) {
      SC32_LOG(error, "server closed during handshake");
      return false;
    }
...
void WebSocketClient::ping() {
  writeFrame(0x9, nullptr, 0);
  awaiting_pong_ = true;
  last_tx_ms_ = timesync::now_ms();
}
// Masked text frame (client -> server)
bool WebSocketClient::writeFrame(uint8_t opcode, const uint8_t* data,
                                 size_t len) {
  if (!open_)
    return false;
  uint8_t mask[4];
  ...
  buf.reserve(2 + 8 + 4 + len);
  buf.push_back(0x80 | (opcode & 0x0F));
  if (len < 126) {
    buf.push_back(0x80 | uint8_t(len));
  } else if (len <= 0xFFFF) {
    buf.push_back(0x80 | 126);
    buf.push_back((len >> 8) & 0xFF);
    buf.push_back(len & 0xFF);
  } else {
    buf.push_back(0x80 | 127);
    for (int i = 7; i >= 0; --i)
      buf.push_back(uint8_t((uint64_t(len) >> (i * 8)) & 0xFF));
  }
  buf.insert(buf.end(), mask, mask + 4);
  size_t off = buf.size();
  buf.resize(off + len);
  for (size_t i = 0; i < len; ++i)
    buf[off + i] = data[i] ^ mask[i & 3];
...
bool WebSocketClient::readFrame(std::vector<uint8_t>& out) {
  ...
  if (masked) {
    if (tls_->read(mask, 4) != 4) {
      SC32_LOG(error, "failed to read mask");
      return false;
    }
  }
  ...
  size_t got = 0;
  while (got < len) {
    ssize_t r = tls_->read(reinterpret_cast<uint8_t*>(&payload[got]),
                           (size_t)(len - got));
    if (r <= 0) {
      BELL_SLEEP_MS(5);
      SC32_LOG(error, "yield to read frame, got=%d", got);
      continue;
    }
    got += (size_t)r;
  }
  ...
  if (opcode == 0x1 || opcode == 0x2) {
    out = {payload.begin(), payload.end()};
    return true;
  }  // text / binary
  if (opcode == 0x9) {
    writeFrame(0xA, reinterpret_cast<const uint8_t*>(payload.data()),
               payload.size());
    return false;
  } else
    SC32_LOG(error, "unknown opcode %d", opcode);
  if (opcode == 0x8) {
    open_ = false;
    if (on_close_)
      on_close_(1000, "server-close");
    return false;
  }
  return false;
}
...
void WebSocketClient::runTask() {
  ...
  while (isRunning_.load()) {
    ...
    if (open_ && awaiting_pong_ &&
        (now - last_rx_ms_) >= keepalive_pong_timeout_ms_) {
      SC32_LOG(error, "timeout: no PONG in %ums (last_rx=%llu, now=%llu)",
               keepalive_pong_timeout_ms_, (unsigned long long)last_rx_ms_,
      ...
      if (tls_)
        tls_->close();
      if (on_close_)
        on_close_(1001, "ping-timeout");  // triggers on_close_ callback
