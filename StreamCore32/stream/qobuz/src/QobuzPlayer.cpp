...
// --- local predefs ---
// --- event helpers ---
static inline std::string build_start_event(
    std::shared_ptr<qobuz::QobuzQueueTrack> track, const std::string& user_id,
    int played_for_s);
static inline std::string build_end_event(
    std::shared_ptr<qobuz::QobuzQueueTrack> track, const std::string& user_id,
    int played_for_s);
// --- HTTP helpers ---
...
static inline std::unique_ptr<bell::HTTPClient::Response> open_at(
    const std::string& url, std::optional<size_t> pos = std::nullopt,
    bool keepAlive = false);
// --- AudioFile helpers ---
static inline size_t ms_to_offset(size_t fileSize, size_t duration_ms,
                                  size_t pos_ms);
// --- FLAC helpers ---
static inline void create_flac_metadata(uint8_t* dst, uint32_t sample_rate,
...
static bool parse_flac_frame_header(bell::HTTPClient::Response* s,
                                    ...
                                    uint32_t& sr, uint8_t& ch, uint8_t& bps,
                                    uint16_t& bs);
static inline size_t fetch_flac_metadata(bell::HTTPClient::Response* s,
                                         uint8_t* dst, size_t n);
static inline bool sync_ff_f8fb(const uint8_t* p, size_t n);
//
//---QobuzPlayer---
QobuzPlayer::QobuzPlayer(std::shared_ptr<AudioControl> audio,
  ...
  feed_->state_callback = [this](uint8_t st) {
    if (st == 1) {
      ...
      current_track_playing->startedPlayingAt =
          player_state.currentPosition.timestamp;
      auto resp = on_qobuz_post_(
          "track", "reportStreamingStart",
          build_start_event(current_track_playing, user_id_, 0), {}, false);
      if (!hb_) {
        hb_ = std::make_unique<Heartbeat>([&]() { sendPlayerState(); });
        hb_->start();
      }
      sendPlayerState();
      if (hb_)
      ...
      player_state.currentPosition.value =
          timesync::now_ms() - player_state.currentPosition.timestamp +
          player_state.currentPosition.value;
      auto resp = on_qobuz_post_(
          "track", "reportStreamingEndJson",
          build_end_event(
              current_track_playing, user_id_,
              (timesync::now_ms() - current_track_playing->startedPlayingAt) /
                  1000),
          {}, false);
      sendPlayerState();
      if (hb_)
      ...
      auto resp = on_qobuz_post_(
          ...
          {}, false);
      if (resp->status() != 200)
        SC32_LOG(info, "reportStreamingEnd %s", resp->body_string().c_str());
    ...
    if (onUiMessage_ && (st == 2 || st == 3)) {
      uint64_t pos_ms = 0;
      if (st == 2) {
        pos_ms = timesync::now_ms() - player_state.currentPosition.timestamp +
                 player_state.currentPosition.value;
      } else {
      ...
      switch (current_track_playing->format) {
        ...
        case qobuz::AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_HI_RES_192:
          ...
          s += std::to_string(sr);
          sr *= 1000;
          if (current_track_playing->sampling_rate - sr > 0)
            s += "." + std::to_string(
                           (current_track_playing->sampling_rate - sr) / 100);
          s += "kHz";
          j["quality"] = s;
          break;
        default:
      ...
      j["track"] = {{"title", current_track_playing->title},
...
bool QobuzPlayer::getStreamInfo(std::string url, size_t& length, size_t& offset,
  ...
  offset = 0;
  while (true) {
    if (!check_http_status(resp->status()))
      break;
    auto ctype = svToString(resp->header("content-type"));
    if (ctype.empty())
      break;
    if (ctype.rfind("audio/", 0) != 0 && ctype != "application/octet-stream")
      break;
    ...
    if (format >= qobuz::AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS) {
      if (!probeFlac(resp.get(), length, buffer, offset))
        break;
    }
...
void QobuzPlayer::runTask() {
  ...
  while (isRunning_.load()) {
    ...
    if (wantRestart_.load() && retries > 0) {
      BELL_SLEEP_MS(50);
      if (retries == 1)
        current_track_buffering->format =
            qobuz::AudioFormat::QOBUZ_QUEUE_FORMAT_MP3;
      else if (current_track_buffering->format >
               qobuz::AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS)
        current_track_buffering->format =
            qobuz::AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS;
      ...
      current_track_buffering = queue_->consumeTrack(
          current_track_buffering, player_state.nextQueueItemId);
      if (!current_track_buffering)
        break;
      if (current_track_buffering->state == qobuz::QueuedTrackState::FAILED)
        continue;
      if (player_state.nextQueueItemId == 0) {
        player_state.has_nextQueueItemId = false;
      } else
        player_state.has_nextQueueItemId = true;
    ...
    if (!getStreamInfo(url, totalSize_, baseOffset_,
                       current_track_buffering->format, flac_header)) {
    ...
    while (!wantStop_.load()) {
      ...
      if (current_track_buffering->wantSkip_.load()) {
        ...
        if (st2 != 206 && st2 != 200) {
          if (st2 == 416) {
            ...
            SC32_LOG(error, "resume HTTP %d", st2);
          BELL_YIELD();
          current_track_buffering->wantSkip_.store(false);
          abortTrack = true;
          break;
        ...
        BELL_YIELD();
        if (respRemaining <= 0)
          respRemaining = std::numeric_limits<size_t>::max();
        current_track_buffering->wantSkip_.store(false);
        if (!initial_seek) {
          on_qobuz_post_(
              "track", "reportStreamingEndJson",
              build_end_event(current_track_buffering, user_id_,
                              (timesync::now_ms() -
                               current_track_buffering->startedPlayingAt) /
                                  1000),
              {}, false);
          sendPlayerState();
          hb_->delay();
      ...
      // FEED buffered
      if (out_pos < in_len) {
        size_t fed = feed_->feedData(buf.get() + out_pos,
                                     (size_t)(in_len - out_pos), tid);
        if (fed > 0) {
          out_pos += (size_t)fed;
          n += (size_t)fed;
        } else
      ...
      if (free_space > HEADROOM) {
        ...
        size_t fileRemaining = (totalSize_ >= 0)
                                   ? (totalSize_ - n)
        ...
        size_t to_read = (size_t)std::min<size_t>(
            std::min<size_t>(PULL_BYTES, free_space - HEADROOM),
            std::min<size_t>(fileRemaining, bodyRemaining));
        if (to_read == 0) {
          ...
          if (eofMode_.load()) {
            ...
            if (st3 == 416) {
              ...
              if (out_pos >= in_len)
                break;
            } else if (st3 != 206 && st3 != 200) {
              SC32_LOG(error, "resume HTTP %d", st3);
              abortTrack = true;
              break;
            }
            respRemaining = totalSize_ - n;
        ...
        if (got != to_read) {
          SC32_LOG(info,
                   "short read got=%lu to_read=%lu respRemaining=%lu "
                   "in_len=%lu buf_cap=%lu totalSize=%lu",
                   got, (unsigned long)to_read, (unsigned long)respRemaining,
                   (unsigned long)in_len, (unsigned long)BUF_CAP,
...
void QobuzPlayer::sendPlayerState() {
  ...
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_RNDR_SRVR_STATE_UPDATED;
  ...
  if (player_state.currentPosition.timestamp != 0) {
    ...
    if (player_state.playingState ==
        qconnect_PlayingState_PLAYING_STATE_PLAYING)
      msg.rndrSrvrStateUpdated.state.currentPosition.value =
          msg.rndrSrvrStateUpdated.state.currentPosition.timestamp -
          player_state.currentPosition.timestamp +
          player_state.currentPosition.value;
  } else
    msg.rndrSrvrStateUpdated.state.currentPosition.has_value = false;
  on_ws_msg_(&msg, 1);
}

bool QobuzPlayer::probeFlac(bell::HTTPClient::Response* s, size_t n,
  ...
  if (filled != 4) {
    SC32_LOG(info, "probe short %lu", filled);
    return false;
  }
  ...
  for (;;) {
    ...
    if (at >= 0) {
      ...
      size_t have_after =
          filled - (size_t)at - 1;  // bytes after 0xFF already in buffer
      if (!parse_flac_frame_header(s, hdr, have_after, sr, ch, bps, bs))
        return false;

      if (!sr)
        sr = current_track_buffering->sampling_rate
                 ? current_track_buffering->sampling_rate
                 : 44100;
      if (!bps)
        bps = current_track_buffering->bits_depth
                  ? current_track_buffering->bits_depth
                  : 16;

      size_t total_samples = 0;  // unknown is fine
      create_flac_metadata(dst, sr, ch ? ch : 2, bps, total_samples,
                           bs ? bs : 4096);
    ...
    if (extra >= MAX_EXTRA) {
      SC32_LOG(info, "no sync within budget");
      return false;
    }
...
static inline size_t parse_content_range_total(const std::string& cr) {
  if (cr.rfind("bytes ", 0) != 0)
    return 0;
  size_t sp = 6;
  size_t dash = cr.find('-', sp);
  if (dash == std::string::npos)
    return 0;
  size_t slash = cr.find('/', dash + 1);
...
static inline std::unique_ptr<bell::HTTPClient::Response> open_at(
    const std::string& url, std::optional<size_t> pos, bool keepAlive) {
  bell::HTTPClient::Headers hdrs = {{"Accept", "audio/*"},
                                    {"Accept-Encoding", "identity"},
                                    {"User-Agent", "StreamCore32/1.0"}};
  if (pos)
    hdrs.emplace_back("Range", "bytes=" + std::to_string(*pos) + "-");
...
static inline void be24(uint8_t* p, uint32_t v) {
  p[0] = uint8_t(v >> 16);
  p[1] = uint8_t(v >> 8);
  p[2] = uint8_t(v);
}
...
static inline void create_flac_metadata(uint8_t* dst, uint32_t sample_rate,
  ...
  const uint64_t bps5 =
      uint64_t((bits_per_sample ? bits_per_sample : 16) - 1) & 0x1F;
...
static bool parse_flac_frame_header(bell::HTTPClient::Response* s,
  ...
  if (ch_code <= 7)
    ...
    ch = 2;  // left/side, right/side, mid/side → 2 channels
  ...
  switch (sz_code) {
    ...
    default:
      bps = 0;
      break;  // unknown → use metadata fallback later
  }
  ...
  switch (bs_code) {
    ...
    case 6: {  // 8-bit (blocksize-1)
      uint8_t ext;
      if (have_after_sync >= 4)
        ext = buf[4];
      else {
        if (s->readExact(&ext, 1) != 1)
          return false;
      }
      bs = uint16_t(ext) + 1;
    } break;
    case 7: {  // 16-bit (blocksize-1)
      ...
      size_t need = 2;
      size_t have =
          (have_after_sync >= 5) ? std::min<size_t>(have_after_sync - 4, 2) : 0;
      if (have)
        memcpy(ext, buf + 4, have);
      if (have < need) {
        if (s->readExact(ext + have, need - have) != (int)(need - have))
          return false;
      }
  ...
  switch (sr_code) {
    ...
    case 12: {  // 8-bit kHz
      ...
      if (s->readExact(&x, 1) != 1)
        return false;
      sr = uint32_t(x) * 1000u;
    } break;
    case 13: {  // 16-bit Hz
      uint8_t x[2];
      if (s->readExact(x, 2) != 2)
        return false;
    ...
    default:
      sr = 0;
      break;  // from STREAMINFO / reserved
  }
  return true;
}
static inline size_t fetch_flac_metadata(bell::HTTPClient::Response* s,
  ...
  while (!isLast && bytesRead < n) {
    ...
    if (got != 4) {
      SC32_LOG(info, "probe short %lu", bytesRead);
      return false;
    }
    bytesRead += got;
    isLast = bh[0] & 0x80;
    ...
    uint32_t bsize =
        (uint32_t)bh[1] << 16 | (uint32_t)bh[2] << 8 | (uint32_t)bh[3];
