...
bool QobuzQueue::getMetadata(std::shared_ptr<QobuzQueueTrack> track) {
  if (!track->id)
    return false;
  auto resp = on_qobuz_get_("track", "get",
                            {{"track_id", std::to_string(track->id)}}, false);
  if (resp->status() == 200) {
    nlohmann::json j =
        nlohmann::json::parse(resp->body_string(), nullptr, false);
    if (j.is_discarded() || j.empty())
      return false;
    return loadMetadata(track, j);
  } else {
...
bool QobuzQueue::loadMetadata(std::shared_ptr<QobuzQueueTrack> track,
                              nlohmann::json& json) {
  if (json.empty())
    return false;

  if (json.find("streamable") == json.end() || json["streamable"] == false)
    return false;
  if (track->format > AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS) {
    if (json.find("hires_streamable") == json.end() ||
        json["hires_streamable"] == false)
      track->format = AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS;
    else {
      uint32_t maxSamplingRate =
          json.find("maximum_sampling_rate") == json.end()
              ? 44100
              : (uint32_t)(json["maximum_sampling_rate"].get<double>() * 1000);
      if (maxSamplingRate <= 44100)
        track->format = AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_LOSSLESS;
      else if (maxSamplingRate <= 96000)
        track->format = AudioFormat::QOBUZ_QUEUE_FORMAT_FLAC_HI_RES_96;
    }
  }
  ...
  if (json.find("album") != json.end()) {
    ...
    if (json["album"].find("image") != json["album"].end()) {
      if (json["album"]["image"].find("large") != json["album"]["image"].end())
        track->album.image.large_img = json["album"]["image"]["large"];
      if (json["album"]["image"].find("small") != json["album"]["image"].end())
        track->album.image.small_img = json["album"]["image"]["small"];
      if (json["album"]["image"].find("thumbnail") !=
          json["album"]["image"].end())
        track->album.image.thumbnail = json["album"]["image"]["thumbnail"];
    }
...
bool QobuzQueue::getFileUrl(std::shared_ptr<QobuzQueueTrack> track) {
  ...
  if (resp->status() == 200) {
    ...
    if (json.empty())
      return false;
    if (json.contains("status") && json["status"] == "error")
      return false;
    if (json.contains("url"))
      track->fileUrl = json["url"].get<std::string>();
    if (json.contains("blob"))
      track->blob = json["blob"].get<std::string>();
    if (json.contains("duration"))
      track->durationMs = (size_t)(json["duration"].get<size_t>() * 1000);
    if (json.contains("n_channels"))
      track->n_channels = json["n_channels"].get<int>();
    if (json.contains("bit_depth"))
      track->bits_depth = json["bit_depth"].get<int>();
    if (json.contains("sampling_rate"))
      track->sampling_rate = (int)(json["sampling_rate"].get<double>() * 1000);
    SC32_LOG(info,
             "QobuzQueue::getFileUrl: ms=%lu, channels=%d, depth=%d, rate=%d",
             track->durationMs, track->n_channels, track->bits_depth,
             track->sampling_rate);
...
static std::string buildSuggestionsPayload(
...
void QobuzQueue::sendCommingTracks() {
  ...
  msg.messageType =
      qconnect_QConnectMessageType_MESSAGE_TYPE_CTRL_SRVR_AUTOPLAY_ADD_TRACKS;
  msg.has_ctrlSrvrAutoplayLoadTracks = true;
  msg.ctrlSrvrAutoplayLoadTracks.trackIds_count = queue_.size() - index_;
  msg.ctrlSrvrAutoplayLoadTracks.trackIds = (uint32_t*)calloc(
      msg.ctrlSrvrAutoplayLoadTracks.trackIds_count, sizeof(uint32_t));
  ...
  msg.ctrlSrvrAutoplayLoadTracks.contextUuid =
      dataToPbArray(sessionId, sizeof(sessionId));
  msg.ctrlSrvrAutoplayLoadTracks.actionUuid =
      vectorToPbArray(pbArrayToVector(queueuState.actionUuid));
...
bool QobuzQueue::getSuggestions() {
  ...
  while (1) {
    auto resp = on_qobuz_post_(
        "dynamic", "suggest",
        buildSuggestionsPayload(queue_, expandedTrackInfo_cache_), {}, false);
    if (resp->status() == 200) {
      ...
      if (json.find("tracks") != json.end()) {
        ...
        if (!tracks.size())
          return false;
        msg.ctrlSrvrAutoplayLoadTracks.trackIds_count =
            tracks.size() + preloadedTracks_.size();
        msg.ctrlSrvrAutoplayLoadTracks.trackIds = (uint32_t*)calloc(
            msg.ctrlSrvrAutoplayLoadTracks.trackIds_count, sizeof(uint32_t));
        ...
        for (int i = 0; i < tracks.size(); i++) {
          msg.ctrlSrvrAutoplayLoadTracks.trackIds[i + preloadedTracks_.size()] =
              tracks[i]["id"];
        }
        msg.ctrlSrvrAutoplayLoadTracks.has_queueVersion = true;
        msg.ctrlSrvrAutoplayLoadTracks.queueVersion = queueuState.queueVersion;
        if (resp->header("set-cookie").size()) {
          ...
          if (cookie.find("qobuz-session") != std::string::npos) {
            std::string session = cookie.substr(
                cookie.find("qobuz-session=") + 14,
                cookie.find(";") - (cookie.find("qobuz-session=") + 14));
            std::vector<uint8_t> session_to_16;
            for (int i = 0; i < session.size(); i += 2) {
              session_to_16.push_back(
                  H2Digit2Dez(session.substr(i, 2).c_str()));
            }
            msg.ctrlSrvrAutoplayLoadTracks.contextUuid =
                vectorToPbArray(session_to_16);
          }
        } else
          msg.ctrlSrvrAutoplayLoadTracks.contextUuid =
              dataToPbArray(sessionId, sizeof(sessionId));
      ...
      SC32_LOG(error, "QobuzQueue::getSuggestions: %s",
    ...
    if (preloadedTracks_.size() < 3 && !fetchedAutoplay_) {
      ...
      for (int i = preloadedTracks_.size(); i < 3; i++) {
        if (index_ + preloadedTracks_.size() < queue_.size()) {
          ...
          preloadedTracks_.push_back(
              std::make_shared<QobuzQueueTrack>(queue_[idx], audioFormat_));
        } else
          break;
      }
      if (!fetchedAutoplay_ && preloadedTracks_.size() < 2 &&
          expandedTrackInfo_cache_.size()) {
        if (!getSuggestions()) {
    ...
    for (auto& track : tracks) {
      ...
      switch (track->state) {
        case QueuedTrackState::QUEUED:
          ...
          if (!getMetadata(track))
            track->state = QueuedTrackState::FAILED;
          else
            done = true;
          break;
        case QueuedTrackState::STREAMABLE:
          ...
          if (!getFileUrl(track))
            ...
            std::string ctx = track->contextJson();
            {
              ...
              if (expandedTrackInfo_cache_.size() > 5)
                expandedTrackInfo_cache_.pop_front();
            }
          }
          break;
        default:
...
std::shared_ptr<QobuzQueueTrack> QobuzQueue::consumeTrack(
    std::shared_ptr<QobuzQueueTrack> prevTrack, int32_t& nextTrackQueueIndex) {
  if (!queue_.size())
    return nullptr;
  while (true) {
    if (fetchedAutoplay_) {
    ...
    std::shared_ptr<qobuz::QobuzQueueTrack> track;
    {
      std::scoped_lock lock(preloadedTracksMutex_);
      if (prevTrack) {
        auto prevTrackIter = std::find(preloadedTracks_.begin(),
                                       preloadedTracks_.end(), prevTrack);
      ...
      if (preloadedTracks_.size() > 1)
        nextTrackQueueIndex = preloadedTracks_[1]->index;
      else
        nextTrackQueueIndex = 0;
      if (index_ >= shuffledIndexes_.size()) {
        shuffledIndexes_.push_back(index_);
      }
    }
    if (track->state != QueuedTrackState::READY &&
        track->state != QueuedTrackState::FAILED) {
      BELL_SLEEP_MS(100);
      continue;
    }
