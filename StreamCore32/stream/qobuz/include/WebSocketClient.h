...
class WebSocketClient : public bell::Task {
 public:
  ...
  bool connect(const std::string& wss_url, const std::string& origin,
  ...
  std::vector<uint8_t> pack(uint8_t kind, const std::vector<uint8_t>& payload) {
    ...
    frame.reserve(1 + 10 +
                  payload.size());  // kind + up to 10 bytes varint + data
    frame.push_back(kind);
    writeVarint(payload.size(), frame);
    frame.insert(frame.end(), payload.begin(), payload.end());
    return frame;
  }
  std::vector<std::pair<uint32_t, std::vector<uint8_t>>> parse(
    ...
    while (off < buf.size()) {
      if (buf.size() - off < 2)
        break;  // need at least kind + 1 byte of len

      uint8_t kind = buf[off++];

      uint64_t len = 0;
      size_t lenStart = off;
      if (!readVarint(buf, off, len)) {
        off = lenStart - 1;
        break;
      }  // wait for more data
      ...
      out.emplace_back(
          ...
          std::vector<uint8_t>(
              &buf[off],
              &buf[off + len]));  //std::string((char*)&buf[off], (size_t)len));
      off += (size_t)len;
    }
 ...
 private:
  bool handshake(const std::string& url, const std::string& origin,
                 const std::vector<std::string>& subprotocols);
  ...
  static inline bool readVarint(const std::vector<uint8_t>& buf, size_t& off,
                                uint64_t& out) {
    out = 0;
    int shift = 0;
