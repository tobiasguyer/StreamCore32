...
BELL_LOG(info, "STREAMCONTROL", "AudioControl::AudioControl() end");
  ...
  this->audioSink->state_callback = [this](_AudioSink::Stream::State state, void* source) {
    if(source == NULL) return;
    if(state == _AudioSink::Stream::State::Playback) {
      if(this->isRunning.load() == false) this->isRunning.store(true);
      this->isPaused.store(false);
    } else if(state == _AudioSink::Stream::State::PlaybackPaused) {
      this->isPaused.store(true);
    } else if(state == _AudioSink::Stream::State::Stopped && this->audioSink->streams.size() == 1) {
      this->isRunning.store(false);
    }
    static_cast<FeedControl*>(source)->state_callback((uint8_t)state);
  };
}
size_t AudioControl::FeedControl::feedData(uint8_t* data, size_t bytes, size_t streamId,
        bool STORAGE_VOLATILE) {     
auto it = std::find_if(
    audioSink->streams.begin(), audioSink->streams.end(),
...
std::shared_ptr<_AudioSink::Stream> stream =
  ...
  if(stream == nullptr){
    stream = std::make_shared<_AudioSink::Stream>(this, streamId, CONFIG_STREAM_BUFFER_SIZE); // Recomended buffer size is a multiple of 1024
  ...
  if( streamId != audioSink->streams[0]->streamId) {
    if(streamId > audioSink->streams[0]->streamId){
      if(audioSink->streams[0]->state != _AudioSink::Stream::State::Stopped && audioSink->streams[0]->state > _AudioSink::Stream::State::Playback)
