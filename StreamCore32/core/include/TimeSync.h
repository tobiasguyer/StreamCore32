...
inline void init(const char* s0 = "pool.ntp.org",
  ...
  if (s0)
    sntp_setservername(0, const_cast<char*>(s0));
  if (s1)
    sntp_setservername(1, const_cast<char*>(s1));
  if (s2)
    sntp_setservername(2, const_cast<char*>(s2));
...
inline bool wait_until_valid(uint32_t timeout_ms = 8000) {
  ...
  while ((static_cast<uint64_t>(esp_timer_get_time()) / 1000ULL) - start_ms <
         timeout_ms) {
...
inline std::string now_s_text(int decimals = 6) {
  ...
  char frac[16];
  std::snprintf(frac, sizeof(frac), "%06ld",
                static_cast<long>(tv.tv_usec));  // 6 digits
  std::string f(frac);
  if (decimals <= 6)
    f.resize(decimals);
  else
    f.append(decimals - 6, '0');

  std::snprintf(buf, sizeof(buf), "%ld.%s", static_cast<long>(tv.tv_sec),
                f.c_str());
  return std::string(buf);
}
