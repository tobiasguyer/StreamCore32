...
inline std::map<std::string, std::string> zc_parseBodyToMap(mg_connection* conn) {
  ...
  if (const auto* ri = mg_get_request_info(conn)) {
    if (ri->content_length > 0) {
      ...
      if (first != std::string::npos && body[first] == '{') {
        try {
          ...
          for (auto it = j.begin(); it != j.end(); ++it) {
            if (it->is_string()) out[it.key()] = it->get<std::string>();
      ...
      for (int i = 0; i < num; ++i) out[hdrs[i].name] = hdrs[i].value;
    }
  }
  return out;
}
...
public:
  ...
  struct Endpoint {
    ...
    std::function<std::string(mg_connection*)> handler;     // returns a JSON string body
  };
  ...
  bool addService(const ServiceSpec& spec) {
    ...
    if (spec.key.empty() || spec.serviceType.empty()) {
      BELL_LOG(error, "zeroconf", "addService: key and serviceType are required");
      return false;
    }
    ...
    for (const auto& ep : spec.endpoints) {
      if (ep.path.empty() || !ep.handler) {
        BELL_LOG(error, "zeroconf", "addService[%s]: invalid endpoint", spec.key.c_str());
        return false;
      }
  ...
  bool removeService(const std::string& key) {
    ...
    if (it->second.mdns) { it->second.mdns->unregisterService(); it->second.mdns.reset(); }
    services_.erase(it);
    return true;
  }

  bool hasService(const std::string& key) const { return services_.count(key) > 0; }
