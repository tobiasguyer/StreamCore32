...
namespace nvsc {
  ...
  inline std::string key_suffix_for(const std::string& userkey) {
    uint8_t hash[32];
    mbedtls_sha256_ret(reinterpret_cast<const unsigned char*>(userkey.data()), userkey.size(), hash, 0);
  ...
  inline void strip_nvs_cstr(std::string& s, size_t nvs_len_including_nul) {
    if (nvs_len_including_nul > 0 && nvs_len_including_nul <= s.size()) s.resize(nvs_len_including_nul - 1);
  }
  ...
  inline std::string b64encode(const std::vector<uint8_t>& in) {
    ...
    std::string out;
    out.resize(olen);
    if (mbedtls_base64_encode(reinterpret_cast<unsigned char*>(&out[0]), out.size(), &olen, in.data(), in.size()) != 0) return {};
    out.resize(olen);
    return out;
  }
  inline std::vector<uint8_t> b64decode(const std::string& in) {
    size_t olen = 0;
    mbedtls_base64_decode(nullptr, 0, &olen, reinterpret_cast<const unsigned char*>(in.data()), in.size());
    std::vector<uint8_t> out;
    out.resize(olen);
    if (mbedtls_base64_decode(out.data(), out.size(), &olen, reinterpret_cast<const unsigned char*>(in.data()), in.size()) != 0) return {};
    out.resize(olen);
    return out;
  }
...
inline esp_err_t gcm_encrypt(const std::vector<uint8_t>& key32,
  ...
  uint8_t iv[kIVLen]; fill_iv(iv);

  mbedtls_gcm_context ctx;
  mbedtls_gcm_init(&ctx);
  int rc = mbedtls_gcm_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, key32.data(), 256);
  if (rc != 0) { mbedtls_gcm_free(&ctx); return ESP_ERR_INVALID_ARG; }
...
inline esp_err_t gcm_decrypt(const std::vector<uint8_t>& key32,
  ...
  if (key32.size() != 32) return ESP_ERR_INVALID_ARG;
  if (len < 4 + 1 + kIVLen + kTagLen) return ESP_ERR_INVALID_SIZE;
  if (memcmp(blob, kMagic, 4) != 0)   return ESP_ERR_INVALID_STATE;

  const uint8_t ver = blob[4]; (void)ver; // currently unused
  ...
  inline std::vector<Field> json_to_fields(cJSON* o) {
    ...
    if (!cJSON_IsObject(o)) return out;
    cJSON* it = nullptr;
  ...
  public:
    ...
    explicit NvsHandle(const char* ns, nvs_open_mode_t mode = NVS_READWRITE) {
      ...
      if (err != ESP_OK) ESP_LOGE(TAG, "nvs_open %s failed: %s", ns, esp_err_to_name(err));
    }
    ~NvsHandle() {
      if (h) nvs_close(h);
    }
  ...
  // Forward decl
  inline esp_err_t load_by_suffix(nvs_handle_t handle, const std::string& suf, Record* out);
  ...
  inline esp_err_t load_by_suffix(nvs_handle_t handle, const std::string& suf, Record* out) {
    ...
    if (err != ESP_OK) return err;
    std::string u(len, '\0');
    if ((err = nvs_get_str(handle, kname.c_str(), &u[0], &len)) != ESP_OK) return err;
    strip_nvs_cstr(u, len);

    size_t bl = 0;
    if ((err = nvs_get_blob(handle, kblob.c_str(), nullptr, &bl)) != ESP_OK) return err;
    std::string js(bl, '\0');
    if ((err = nvs_get_blob(handle, kblob.c_str(), &js[0], &bl)) != ESP_OK) return err;
  ...
  inline bool find_suffix_for_userkey(nvs_handle_t handle, const std::string& userkey, std::string& out_suf) {
    nvs_iterator_t it = nvs_entry_find("nvs", DEFAULT_NS, NVS_TYPE_STR);
    while (it) {
      nvs_entry_info_t info;
      nvs_entry_info(it, &info);
  ...
  inline esp_err_t search_userkeys(const std::string& query, std::vector<std::string>& out_userkeys, bool case_insensitive = true, bool prefix_only = false) {
    out_userkeys.clear();
    NvsHandle h(DEFAULT_NS, NVS_READONLY);
    if (!h.ok()) return h.err;
      ...
      if (strncmp(info.key, "n_", 2) == 0) {
        ...
        if (nvs_get_str(h, info.key, nullptr, &len) == ESP_OK) {
          ...
          if (nvs_get_str(h, info.key, &u[0], &len) == ESP_OK) {
            ...
            bool match = prefix_only ? (cmp.rfind(q, 0) == 0) : (q.empty() || cmp.find(q) != std::string::npos);
            if (match) out_userkeys.push_back(std::move(u));
          }
        }
      }
    }
  ...
  inline bool exists(const std::string& userkey) {
    ...
    size_t l = 0;
    if (nvs_get_str(h, key_user(key_suffix_for(userkey)).c_str(), nullptr, &l) == ESP_OK) {
      std::string u(l, '\0');
      if (nvs_get_str(h, key_user(key_suffix_for(userkey)).c_str(), &u[0], &l) == ESP_OK) {
        strip_nvs_cstr(u, l);
        if (u == userkey) return true;
      }
    }
    std::string suf;
  ...
  inline esp_err_t erase(const std::string& userkey) {
    ...
    if (!h.ok()) return h.err;
    std::string suf;
    if (!find_suffix_for_userkey(h, userkey, suf)) return ESP_ERR_NOT_FOUND;
    ...
    if (nvs_get_str(h, CURR_KEY, curr, &l) == ESP_OK) {
      ...
      if (cu == userkey) nvs_erase_key(h, CURR_KEY);
    }
    if (e1 == ESP_OK || e2 == ESP_OK) return nvs_commit(h);
  ...
  inline esp_err_t get_current(Record* out) {
    ...
    std::string u(l, '\0');
    if ((e = nvs_get_str(h, CURR_KEY, &u[0], &l)) != ESP_OK) return e;
    strip_nvs_cstr(u, l);
    return load(u, out);
  }

  // ------------ field-oriented helpers (modify single field efficiently) -----
  inline esp_err_t update_field(const std::string& userkey, const std::string& field_name, const std::vector<uint8_t>& value, bool create_record_if_missing = false) {
    BELL_LOG(info, "Update field '%s' for userkey '%s' in namespace '%s'", field_name.c_str(), userkey.c_str(), DEFAULT_NS);
    if (field_name.empty()) return ESP_ERR_INVALID_ARG;
    NvsHandle h(DEFAULT_NS, NVS_READWRITE);
    if (!h.ok()) return h.err;
    ...
    if (!found) {
      if (!create_record_if_missing) return ESP_ERR_NOT_FOUND;
      Record r{ userkey, {} };
    ...
    if (nvs_get_blob(h, kblob.c_str(), nullptr, &bl) == ESP_OK) {
      js.resize(bl);
      ESP_RETURN_ON_ERROR(nvs_get_blob(h, kblob.c_str(), &js[0], &bl), TAG, "get_blob");
    }
    cJSON* root = js.empty() ? cJSON_CreateObject() : cJSON_ParseWithLength(js.data(), js.size());
    if (!root) root = cJSON_CreateObject();

    // set/replace field
    std::string b64 = b64encode(value);
    cJSON_ReplaceItemInObject(root, field_name.c_str(), cJSON_CreateString(b64.c_str()));
    ...
    // ensure name
    if (err == ESP_OK) err = nvs_set_blob(h, kblob.c_str(), jsw, strlen(jsw));
    cJSON_free(jsw);
    cJSON_Delete(root);
    if (err != ESP_OK) return err;
    return nvs_commit(h);
  ...
  inline esp_err_t get_field(const std::string& userkey, const std::string& field_name, std::vector<uint8_t>* out) {
    if (!out || field_name.empty()) return ESP_ERR_INVALID_ARG;
    Record r;
  ...
  inline esp_err_t set_field_str(const std::string& userkey, const std::string& field_name, const std::string& value, bool create_record_if_missing = false) {
    return update_field(userkey, field_name, as_bytes(value), create_record_if_missing);
  }
  inline esp_err_t get_field_str(const std::string& userkey, const std::string& field_name, std::string* out) {
    if (!out) return ESP_ERR_INVALID_ARG;
  ...
  inline esp_err_t import_from_spiffs(const char* path, bool overwrite = true) {
    ...
    if (!root) return ESP_FAIL;
    auto handle_obj = [&](cJSON* o) { cJSON* uk = cJSON_GetObjectItem(o, "userkey");
    ...
    if (cJSON_IsString(uk) && (cJSON_IsObject(fs) || fs == nullptr)) {
      ...
      if (!r.userkey.empty()) save(r, overwrite);
    }
      };
    if (cJSON_IsArray(root)) {
      ...
      for (int i = 0;i < sz;++i) {
        ...
        if (cJSON_IsObject(o)) handle_obj(o);
      }
    }
  ...
  inline esp_err_t get_first(Record* out) {
    ...
    while (it) {
      ...
      if (strncmp(info.key, "n_", 2) == 0) {
        ...
        if (best.empty() || (std::string("n_") + suf) < (std::string("n_") + best)) best = std::move(suf);
      }
    } if (best.empty()) return ESP_ERR_NOT_FOUND;
  ...
  inline esp_err_t get_startup_record(Record* out, bool set_as_current = true) {
    ...
    e = get_first(out);
    if (e == ESP_OK && set_as_current) set_current(out->userkey);
    return e;
  }
  ...
  public:
    ...
    esp_err_t save(const Record& r, bool overwrite = true) const {
      ...
      if (!overwrite) {
        size_t len = 0;
        if (nvs_get_str(h, kname.c_str(), nullptr, &len) == ESP_OK) return ESP_ERR_INVALID_STATE;
      }
      cJSON* root = fields_to_json(r.fields);
      char* js = cJSON_PrintUnformatted(root);
      if (!js) {
      ...
      esp_err_t err = nvs_set_str(h, kname.c_str(), r.userkey.c_str());
      if (err == ESP_OK) err = nvs_set_blob(h, kblob.c_str(), js, strlen(js));
      cJSON_free(js);
      cJSON_Delete(root);
      if (err != ESP_OK) return err;
      return nvs_commit(h);
      ...
      if (err != ESP_OK) {
        ...
        while (it) {
          ...
          if (strncmp(info.key, "n_", 2) == 0) {
            size_t len = 0;
            if (nvs_get_str(h, info.key, nullptr, &len) == ESP_OK) {
              std::string u(len, '\0');if (nvs_get_str(h, info.key, &u[0], &len) == ESP_OK) {
                strip_nvs_cstr(u, len);
                if (u == userkey) {
                  return load_by_suffix(h, std::string(info.key + 2), out);
                }
    ...
    esp_err_t search_userkeys(const std::string& query, std::vector<std::string>& out_userkeys, bool case_insensitive = true, bool prefix_only = false) const {
      out_userkeys.clear();
      NvsHandle h(ns_, NVS_READONLY);
      if (!h.ok()) return h.err;
      std::string q = case_insensitive ? to_lower(query) : query;
    ...
    // field helpers
    esp_err_t update_field(const std::string& userkey, const std::string& field_name, const std::vector<uint8_t>& value, bool create_record_if_missing = false) const {
      BELL_LOG(info, "Update field '%s' for userkey '%s' in namespace '%s'", field_name.c_str(), userkey.c_str(), ns_);
      if (field_name.empty()) return ESP_ERR_INVALID_ARG;
      NvsHandle h(ns_, NVS_READWRITE);
      if (!h.ok()) return h.err;
      std::string suf = key_suffix_for(userkey);
      ...
      if (nvs_get_blob(h, kblob.c_str(), nullptr, &bl) == ESP_OK) {
        js.resize(bl);
        if (nvs_get_blob(h, kblob.c_str(), &js[0], &bl) != ESP_OK) return ESP_FAIL;
      }
      cJSON* root = js.empty() ? cJSON_CreateObject() : cJSON_ParseWithLength(js.data(), js.size());
      if (!root) root = cJSON_CreateObject();
      std::string b64 = b64encode(value);
    ...
    esp_err_t get_field(const std::string& userkey, const std::string& field_name, std::vector<uint8_t>* out) const {
      if (!out || field_name.empty()) return ESP_ERR_INVALID_ARG;
      Record r;
      esp_err_t e = load(userkey, &r);
      if (e != ESP_OK) return e;
    ...
    esp_err_t set_field_str(const std::string& userkey, const std::string& field_name, const std::string& value, bool create_record_if_missing = false) const {
      return update_field(userkey, field_name, as_bytes(value), create_record_if_missing);
    }
    esp_err_t get_field_str(const std::string& userkey, const std::string& field_name, std::string* out) const {
      if (!out) return ESP_ERR_INVALID_ARG;
      std::vector<uint8_t> v;
      esp_err_t e = get_field(userkey, field_name, &v);
      if (e != ESP_OK) return e;
  ...
  public:
    ...
    esp_err_t set_master_key(const uint8_t* key, size_t len) {
      if (!key || len != 32) return ESP_ERR_INVALID_ARG;
    ...
    esp_err_t save(const Record& r, bool overwrite = true) const {
      if (!has_key_) return ESP_ERR_INVALID_STATE;
      const std::string suf = key_suffix_for(r.userkey);
      const std::string kname = key_user(suf);
      const std::string kblob = key_blob(suf);
      NvsHandle h(ns_, NVS_READWRITE);
      ...
      esp_err_t ce = gcm_encrypt(master_key_, std::string(js, js + strlen(js)), payload);
      cJSON_free(js);
      cJSON_Delete(root);
      if (ce != ESP_OK) return ce;
      esp_err_t err = nvs_set_str(h, kname.c_str(), r.userkey.c_str());
      if (err == ESP_OK) err = nvs_set_blob(h, kblob.c_str(), payload.data(), payload.size());
      if (err != ESP_OK) return err;
      return nvs_commit(h);
    }
    ...
    found:
      {
        size_t bl = 0;
        if (nvs_get_blob(h, key_blob(suffix).c_str(), nullptr, &bl) != ESP_OK) return ESP_ERR_NOT_FOUND;
        std::vector<uint8_t> blob(bl);
        if (nvs_get_blob(h, key_blob(suffix).c_str(), blob.data(), &bl) != ESP_OK) return ESP_FAIL;
        if (!blob_is_encrypted(blob.data(), blob.size())) return ESP_ERR_INVALID_STATE;
        std::string js;
    ...
    esp_err_t search_userkeys(const std::string& query, std::vector<std::string>& out_userkeys, bool ci = true, bool prefix_only = false) const {
      out_userkeys.clear();
      NvsHandle h(ns_, NVS_READONLY);
      if (!h.ok()) return h.err;
    ...
    // field helpers
    esp_err_t update_field(const std::string& userkey, const std::string& field_name, const std::vector<uint8_t>& value, bool create_if_missing = false) const {
      BELL_LOG(info,"SecureStore::update_field(userkey='%s', field_name='%s', value_len=%u, create_if_missing=%d)", userkey.c_str(), field_name.c_str(), (unsigned)value.size(), (int)create_if_missing); 
      if (!has_key_) return ESP_ERR_INVALID_STATE;
      if (field_name.empty()) return ESP_ERR_INVALID_ARG;
      NvsHandle h(ns_, NVS_READWRITE);
      if (!h.ok()) return h.err;
      ...
      if (!found) {
        if (!create_if_missing) return ESP_ERR_NOT_FOUND;
        Record r{ userkey,{} };
        ESP_ERROR_CHECK_WITHOUT_ABORT(save(r, /*overwrite=*/true));
      }
      size_t bl = 0;
      ...
      if (nvs_get_blob(h, kblob.c_str(), nullptr, &bl) == ESP_OK) {
        blob.resize(bl);
        if (nvs_get_blob(h, kblob.c_str(), blob.data(), &bl) != ESP_OK) return ESP_FAIL;
        if (!blob_is_encrypted(blob.data(), blob.size())) return ESP_ERR_INVALID_STATE;
        if (gcm_decrypt(master_key_, blob.data(), blob.size(), js) != ESP_OK) return ESP_ERR_INVALID_RESPONSE;
      }
      cJSON* root = js.empty() ? cJSON_CreateObject() : cJSON_ParseWithLength(js.data(), js.size());
      if (!root) root = cJSON_CreateObject();
      std::string b64 = b64encode(value);
      ...
      std::vector<uint8_t> payload;
      esp_err_t ce = gcm_encrypt(master_key_, std::string(jsw, jsw + strlen(jsw)), payload);
      cJSON_free(jsw);
      cJSON_Delete(root);
      if (ce != ESP_OK) return ce;
      esp_err_t err = nvs_set_str(h, kname.c_str(), userkey.c_str());
    ...
    esp_err_t set_field_str(const std::string& userkey, const std::string& field_name,
      const std::string& value, bool create_if_missing = false) const {
      return update_field(userkey, field_name, as_bytes(value), create_if_missing);
    }
    esp_err_t get_field_str(const std::string& userkey, const std::string& field_name, std::string* out) const {
      if (!out) return ESP_ERR_INVALID_ARG;
      std::vector<uint8_t> v;
